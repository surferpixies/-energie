...
function discoverySectionHtml(report){
 const maturity=report?.maturity||{icon:"🌱",label:"Ton journal apprend encore",days:0,feelingDays:0};
 const observations=report?.observations||[];
 const cards=observations.length?observations.map((x,i)=>`<article class="card discovery-card"><div class="discovery-card-top"><div class="discovery-icon">${x.icon}</div><span class="discovery-level ${x.confidence.cls}">${x.confidence.icon} ${esc(x.confidence.label)}</span></div><h3>${esc(x.title)}</h3><p>${esc(x.text)}</p><div class="discovery-stat"><strong>${esc(x.statistic)}</strong><span>dans les journées observées</span></div><div class="discovery-meta"><span>observé sur ${x.samples.exposed} journées</span><button class="text-button why-discovery" data-discovery="${i}">Voir pourquoi</button></div></article>`).join(''):`<section class="card discovery-empty"><div class="food-art">🌱</div><h3>Ton journal apprend encore</h3><p>Continue à noter tes repas et tes ressentis. Énergie affichera une tendance seulement lorsqu’il y aura assez de journées comparables et une différence suffisamment nette.</p></section>`;
 return `<section class="discovery-section"><div class="discovery-heading"><div><p class="eyebrow">Observations personnelles</p><h2>Ce que ton journal commence à révéler</h2></div><span class="discovery-count">${observations.length}/3</span></div><div class="journal-maturity"><span>${maturity.icon}</span><div><strong>${esc(maturity.label)}</strong><small>${maturity.feelingDays} journée${maturity.feelingDays!==1?'s':''} avec un ressenti analysé${maturity.feelingDays!==1?'s':''}</small></div></div><div class="discovery-grid">${cards}</div><p class="discovery-disclaimer">Ces observations décrivent des associations dans ton propre journal. Elles ne prouvent pas une cause et ne constituent pas un diagnostic.</p></section>`
}
function openDiscoveryWhy(x){
 $("#sourceTitle").textContent="Pourquoi cette tendance apparaît-elle?";
 $("#sourceContent").innerHTML=`<p>${esc(x.basis||"Cette tendance utilise uniquement les données disponibles dans ton journal.")}</p><div class="discovery-comparison"><div><strong>${esc(x.statistic)}</strong><span>avec cette habitude</span></div><div><strong>${esc(x.comparisonStatistic)}</strong><span>dans les autres journées</span></div></div><div class="notice"><strong>À interpréter avec prudence</strong><p>Une association ne signifie pas qu’un aliment ou une habitude est la cause du ressenti. Le contexte, les portions et d’autres facteurs peuvent varier.</p></div><p class="muted small">Le moteur préfère ne rien afficher lorsqu’il manque de données comparables ou que la différence est trop faible.</p>`;
 $("#sourceDialog").showModal()
}
function renderInsights(){const realMeals=allMeals(),usePreview=!db.settings.demoMode&&realMeals.length<8&&sessionStorage.getItem('dashboardPreview')!=="off";const demo=[{date:todayKey(),time:"07:30",type:"Déjeuner",description:"Yogourt grec, bleuets et granola",fatigueBefore:3},{date:todayKey(),time:"12:10",type:"Dîner",description:"Poulet, riz et brocoli",fatigueBefore:2},{date:todayKey(),time:"18:20",type:"Souper",description:"Pizza et salade",fatigueBefore:3},{date:"2026-07-18",time:"07:45",type:"Déjeuner",description:"Céréales sucrées et jus",fatigueBefore:3},{date:"2026-07-18",time:"12:30",type:"Dîner",description:"Poulet, riz et brocoli",fatigueBefore:2},{date:"2026-07-17",time:"18:00",type:"Souper",description:"Poutine",fatigueBefore:4},{date:"2026-07-16",time:"07:20",type:"Déjeuner",description:"Yogourt grec, bleuets et granola",fatigueBefore:2},{date:"2026-07-15",time:"12:05",type:"Dîner",description:"Poulet, riz et brocoli",fatigueBefore:2},{date:"2026-07-14",time:"18:40",type:"Souper",description:"Pizza",fatigueBefore:3},{date:"2026-07-13",time:"08:10",type:"Déjeuner",description:"Œufs et rôties",fatigueBefore:3}];const meals=usePreview?demo:realMeals,story=dashboardStory(meals);const discoveryReport=window.EnergieObservationEngine?window.EnergieObservationEngine.analyze(db,{meals,limit:3,lookbackDays:180}):{observations:[],maturity:{icon:"🌱",label:"Ton journal apprend encore",feelingDays:0}};const negativeFeelings=historyNegativeFeelingStats(meals);let insights=db.settings.insightsEnabled?buildPersonalInsights(meals):[];insights=insights.concat(buildNutritionObservations(meals));const previewBanner=realMeals.length<8?`<section class="preview-banner"><div><strong>${usePreview?'👀 Mode aperçu activé':'📊 Tes vraies données'}</strong><p>${usePreview?'Des données exemples montrent la présentation. Elles ne sont jamais sauvegardées.':'Les observations utilisent seulement tes repas enregistrés.'}</p></div><button class="secondary small" id="togglePreview">${usePreview?'Voir mes données':'Voir l’aperçu'}</button></section>`:"";$("#app").innerHTML=`<section class="hero"><p class="eyebrow">Tableau intelligent</p><h2>Ce qu’Énergie apprend sur toi</h2><p>Avec les données recueillies, Énergie fait ressortir des habitudes possibles sans jugement, sans diagnostic et sans prétendre expliquer leurs causes.</p></section>${previewBanner}${discoverySectionHtml(discoveryReport)}<div class="grid dashboard-overview"><section class="card stat-card compact-stat-card compact-row-card dashboard-hero-card"><div class="stat-card-heading"><span>🍎</span><div><h3>Tu utilises Énergie depuis</h3><p class="muted small">Date de départ du journal</p></div></div><div class="metric metric-small">${esc(story.since)}</div></section><section class="card stat-card dashboard-mini-card"><span>⭐</span><h3>Point fort</h3><p>${esc(story.strength)}</p></section><section class="card stat-card dashboard-mini-card"><span>💡</span><h3>Habitude observée</h3><p>${esc(story.habit)}</p></section><section class="card stat-card dashboard-mini-card"><span>🎯</span><h3>Suggestion principale</h3><p>${esc(story.suggestion)}</p></section></div>${dashboardNegativeFeelingHtml(negativeFeelings)}${professionalDiscussionHtml(meals)}<div class="section-title"><h2>🧠 Observations</h2><span class="muted small">${insights.length} carte${insights.length>1?'s':''}</span></div><div class="insight-grid">${insights.length?insights.map(insightHtml).join(''):`<section class="card empty wide"><div class="food-art">🧠</div><p>${db.settings.insightsEnabled||db.settings.nutritionObservations?'Continue d’enregistrer tes repas pour obtenir des observations.':'Les observations sont désactivées dans les paramètres.'}</p></section>`}</div>${demoDiscoveryHtml()}${usePreview&&!db.settings.demoMode?'<p class="preview-footnote">Les valeurs du mode aperçu sont fictives et servent uniquement à prévisualiser la présentation.</p>':''}`;$("#togglePreview")?.addEventListener('click',()=>{sessionStorage.setItem('dashboardPreview',usePreview?'off':'on');renderInsights()});$$('.why-insight').forEach(b=>b.onclick=()=>openInsightWhy(insights[Number(b.dataset.insight)]));$$('.why-discovery').forEach(b=>b.onclick=()=>openDiscoveryWhy(discoveryReport.observations[Number(b.dataset.discovery)]))}
