...
function estimateNutritionFromText(text){
 ...
 const total=matched.reduce((a,{food:f})=>({calories:a.calories+(Number(f.calories)||0),protein:a.protein+(Number(f.protein)||0),carbs:a.carbs+(Number(f.carbs)||0),fat:a.fat+(Number(f.fat)||0)}),{calories:0,protein:0,carbs:0,fat:0});
 const portions=[...new Set(matched.map(x=>x.food.portion).filter(Boolean))];
...
function dailyMacroSummaryHtml(meals){
  ...
  const known=[];
  meals.forEach(meal=>{const nutrition=normalNutrition(meal.nutrition)||estimateNutritionFromText(meal.description||"");if(nutrition)known.push(nutrition)});
  ...
  return `<section class="daily-macros-card" aria-label="Macros estimées de la journée"><div class="daily-macros-head"><span>Macros estimées</span><strong>≈ ${calories.toLocaleString('fr-CA')} kcal</strong></div><div class="daily-macros-grid"><span><b>🥩 ${protein.toLocaleString('fr-CA')} g</b><small>Protéines</small></span><span><b>🍞 ${carbs.toLocaleString('fr-CA')} g</b><small>Glucides</small></span><span><b>🥑 ${fat.toLocaleString('fr-CA')} g</b><small>Lipides</small></span></div><p>Valeurs approximatives, calculées à partir des aliments reconnus. ${coverage}</p></section>`;
}
