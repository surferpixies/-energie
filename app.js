...
function formatSleepDuration(hours) {
  ...
  function migrate(raw) {
    ...
    out.settings.calorieTargetMode = ["estimated", "fixed"].includes(out.settings.calorieTargetMode)
      ? out.settings.calorieTargetMode
      : "estimated";
    const migratedFixedCalorieTarget = Number(out.settings.fixedCalorieTarget);
    out.settings.fixedCalorieTarget = Number.isFinite(migratedFixedCalorieTarget) && migratedFixedCalorieTarget > 0
      ? Math.round(migratedFixedCalorieTarget)
      : null;
  ...
  function formDraftSnapshot(form) {
    ...
    return {
      ...
      scoredItems,
      ...(form.id === "mealForm"
        ? { mealPhotos: mealPhotoDrafts.slice(0, 3).map((photo) => ({ ...photo })) }
        : {}),
    };
  }
  ...
  function restoreFormDraft(form) {
    ...
    if (form.id === "mealForm") {
      if (Array.isArray(draft.mealPhotos) && draft.mealPhotos.length) {
        mealPhotoDrafts = draft.mealPhotos.slice(0, 3).map((photo) => ({ ...photo }));
        photoData = mealPhotoDrafts[0]?.local || mealPhotoDrafts[0]?.url || null;
        showPhotoPreview();
        hydrateMealPhotoDraftUrls();
      }
  ...
  async function uploadPhoto(meal) {
    ...
    for (let index = 0; index < photos.length; index += 1) {
      ...
      photo.path = path;
      photo.local = null;
      // Conserver immédiatement une URL d’affichage après l’envoi. Sans cela,
      // le repas pouvait rouvrir avec seulement le chemin Storage et afficher
      // un emplacement vide jusqu’au prochain rafraîchissement des URL signées.
      photo.url = await signedPhoto(path);
    }
    meal.photos = photos;
        ...
        if (op.kind === "day") {
          ...
          const dayPayload = {
            ...
            supplements: {
              ...
              // envoyer les data URL temporaires dans daily_logs. Les photos d’un
              // repas terminé utilisent Supabase Storage via le flux normal.
              formDrafts: Object.fromEntries(
                Object.entries(d.formDrafts || {}).map(([key, draft]) => [
                  key,
                  draft && typeof draft === "object"
                    ? Object.fromEntries(Object.entries(draft).filter(([field]) => field !== "mealPhotos"))
                    : draft,
                ]),
              ),
  ...
  async function pullCloud(show = true) {
    ...
    for (const r of dr.data || []) {
      ...
      if (remoteIsNewer) {
        ...
        if (["estimated", "fixed"].includes(r.supplements?.calorieTargetMode)) db.settings.calorieTargetMode = r.supplements.calorieTargetMode;
        if (Object.prototype.hasOwnProperty.call(r.supplements || {}, "fixedCalorieTarget")) {
          const remoteFixedTarget = Number(r.supplements.fixedCalorieTarget);
          db.settings.fixedCalorieTarget = Number.isFinite(remoteFixedTarget) && remoteFixedTarget > 0
            ? Math.round(remoteFixedTarget)
            : null;
        }
  ...
  function updateMealCompositionReview() {
    ...
    if (db?.settings?.showRecognizedElements === false) {
      section.hidden = true;
      return;
    }
  ...
  function allMealsFromDB(source = db) {
    const cutoff = source?.settings?.demoMode
      ? (demoAnalysisContext()?.cutoff || selectedDate || todayKey())
      : todayKey();
    return Object.values(source?.days || {})
      .flatMap((day) => (Array.isArray(day?.meals) ? day.meals : []))
      .filter((meal) => meal?.description && (!meal?.date || meal.date <= cutoff));
  }
  ...
  function render() {
    ...
    const maxAllowedJournalDate = journalMaxDate();
  ...
  function journalMaxDate() {
    if (db.settings?.demoMode) {
      ...
      return c?.last || todayKey();
    }
    return db.settings?.futureMealPlanning === true
      ? addDaysKey(todayKey(), 2)
      : todayKey();
  }
  function changeJournalDay(offset) {
    ...
    } else if (nextDate > journalMaxDate()) return;
  ...
  function journalSummaryHtml(day, meals) {
    ...
    return `<section class="journal-summary" aria-labelledby="journalSummaryTitle"><div class="journal-summary-intro"><div><p class="eyebrow">Ajout rapide</p><h2 id="journalSummaryTitle">Que veux-tu noter?</h2></div></div><div class="journal-summary-grid">${sleepPrompt}<button type="button" class="journal-summary-action journal-summary-action--meal" id="journalSummaryMeal"><span>Ajouter un</span><strong>Repas</strong><span class="summary-action-watermark" aria-hidden="true">🍲</span><span class="summary-action-plus" aria-hidden="true">+</span><small>${meals.length} repas ou collation${meals.length !== 1 ? "s" : ""} cette journée</small></button><button type="button" class="journal-summary-action journal-summary-action--feeling ${dueFeeling ? "needs-brain-action feeling-needs-brain-action" : ""}" id="journalSummaryFeeling">${feelingBrain}<span class="feeling-summary-copy"><span>${dueFeeling ? "À compléter" : "Ajouter un"}</span><strong>Ressenti</strong><small>${dueFeeling ? "Un ressenti après est maintenant attendu" : feelingCount ? `${feelingCount} repas documenté${feelingCount > 1 ? "s" : ""}` : "Avant, après ou hors repas"}</small></span><span class="summary-feeling-visual" aria-hidden="true" style="position:absolute!important;right:.78rem!important;top:.72rem!important;width:96px!important;height:66px!important;z-index:0!important;pointer-events:none!important;opacity:.88!important;"><svg viewBox="0 0 120 82" role="presentation" style="display:block;width:100%;height:100%;overflow:visible"><defs><radialGradient id="feelingSadGradient" cx="38%" cy="34%" r="72%"><stop offset="0" stop-color="#aaa6bd"/><stop offset="1" stop-color="#8d88a6"/></radialGradient><radialGradient id="feelingGoodGradient" cx="35%" cy="30%" r="76%"><stop offset="0" stop-color="#ffe7c9"/><stop offset="1" stop-color="#f4bb88"/></radialGradient></defs><circle cx="43" cy="42" r="29" fill="url(#feelingSadGradient)" opacity=".88"/><circle cx="78" cy="40" r="31" fill="url(#feelingGoodGradient)" opacity=".96"/><path d="M25 31q7 5 14 0" fill="none" stroke="#515064" stroke-width="2.5" stroke-linecap="round"/><path d="M43 31q7 5 13 0" fill="none" stroke="#515064" stroke-width="2.5" stroke-linecap="round"/><path d="M31 56q10-7 19 0" fill="none" stroke="#515064" stroke-width="2.6" stroke-linecap="round"/><path d="M58 30q7 5 14 0" fill="none" stroke="#7c5741" stroke-width="2.4" stroke-linecap="round"/><path d="M80 30q7 5 14 0" fill="none" stroke="#7c5741" stroke-width="2.4" stroke-linecap="round"/><path d="M66 53q10 8 20 0" fill="none" stroke="#7c5741" stroke-width="2.8" stroke-linecap="round"/><ellipse cx="60" cy="45" rx="5.5" ry="3" fill="#e9977d" opacity=".28"/><ellipse cx="94" cy="45" rx="5.5" ry="3" fill="#e9977d" opacity=".28"/></svg></span><span class="summary-action-plus" aria-hidden="true">+</span></button>${summaryActivityHtml(day)}${summaryHydrationHtml(day)}${stepsProgressHtml(day, true)}</div></section>`;
  }
  ...
  function renderToday() {
    ...
    const latestDate = demoRange?.last || journalMaxDate();
    const isToday = selectedDate === todayKey();
    const isFuture = !db.settings?.demoMode && selectedDate > todayKey();
    const sleepPct =
      d.sleepHours == null
        ? 0
  ...
  function relativeDayLabel(date) {
    ...
    if (date === addDaysKey(todayKey(), 1)) return "Demain";
    if (date === addDaysKey(todayKey(), 2)) return "Après-demain";
  ...
  function bindObservationExplorer() {
    ...
    card.onclick = (event) => {
      ...
      if (why && card.contains(why)) {
        ...
        const wrap = why.closest(".observation-explorer-why");
        const body = wrap?.querySelector(".observation-explorer-why-body");
        if (!body) return;
  ...
  function personalProfileHtml() {
    const profile = personalProfile(), age = profile.age || {}, height = profile.height || {}, weight = profile.weight || {}, sex = profile.sex || {}, unit = weight.unit || "kg";
  ...
  function bindPersonalProfile() {
    ...
    function saveHeight() {
      ...
      const raw = heightInput.value.trim(), height = Metrics.number(raw), valid = !raw || (height != null && height >= 30 && height <= 250);
      $("#profileHeightError").textContent = valid ? "" : "Entre une taille entre 30 et 250 cm.";
      ...
      setPersonalRecord("height", {mode: "provided", value: raw ? height : null});
    }
    function saveWeight() {
  ...
  function weightKgForDate(date) {
    const entries = Object.entries(db.days || {})
      .filter(([key, day]) => key <= date && Metrics.dateTime(key) != null && Metrics.weightRecord(day?.weightMeasurement)?.kg != null)
      .sort(([a], [b]) => b.localeCompare(a));
    return entries.length ? Metrics.weightRecord(entries[0][1].weightMeasurement).kg : null;
  }
  function estimatedRestingCalories(profile, weightKg) {
    const age = Number(profile.age?.value), height = Number(profile.height?.value), sex = profile.sex?.value;
    if (profile.age?.mode !== "provided" || profile.height?.mode !== "provided" || profile.sex?.mode !== "provided") return null;
    if (!Number.isFinite(age) || !Number.isFinite(height) || !Number.isFinite(weightKg) || !["female", "male"].includes(sex)) return null;
  ...
  function energyBalanceProfileReady(profile) {
    return profile.age?.mode === "provided" && Number.isFinite(Number(profile.age?.value)) &&
      profile.height?.mode === "provided" && Number.isFinite(Number(profile.height?.value)) &&
      profile.weight?.mode === "provided" && latestPersonalWeight()?.kg != null &&
      profile.sex?.mode === "provided" && ["female", "male"].includes(profile.sex?.value);
  }

  function calorieTargetMode() {
    return db.settings?.calorieTargetMode === "fixed" ? "fixed" : "estimated";
  }
  function fixedCalorieTarget() {
    const value = Number(db.settings?.fixedCalorieTarget);
  ...
  function calorieTargetGaugeHtml(date, meals, options = {}) {
    ...
    if ((mode === "estimated" && !estimated) || (mode === "fixed" && !fixed)) {
      const title = mode === "fixed" ? "Cible fixe à compléter" : "Estimation à compléter";
      const detail = mode === "fixed"
        ? "Entre ta cible calorique quotidienne dans Profil."
        : "Ajoute âge, sexe, taille et poids dans Profil.";
      if (options.circular)
        return `<div class="calorie-target-ring-card is-unavailable"><div class="calorie-target-ring"><div class="calorie-target-ring-center"><strong>${consumed ? consumed.toLocaleString("fr-CA") : "—"}</strong><small>kcal</small></div></div><div class="calorie-target-ring-meta"><strong>${title}</strong><small>${detail}</small></div></div>`;
      return `<div class="calorie-target-gauge is-unavailable"><div class="calorie-target-gauge-head"><span>${mode === "fixed" ? "Cible calorique fixe" : "Dépense quotidienne estimée"}</span><strong>À compléter</strong></div><small>${detail}</small></div>`;
    }

    const lead = future ? "Planifié" : "Enregistré", partial = summary.partial ? " · total partiel" : "";

    if (mode === "fixed") {
      const target = fixed;
      const ratio = target > 0 ? consumed / target : 0, pct = Math.max(0, Math.min(100, Math.round(ratio * 100)));
      const remaining = target - consumed;
      const status = remaining >= 0
        ? `≈ ${remaining.toLocaleString("fr-CA")} kcal avant la cible fixe`
        : `≈ ${Math.abs(remaining).toLocaleString("fr-CA")} kcal au-delà de la cible fixe`;
      if (options.circular) {
        const metaTitle = options.daily
          ? `${consumed.toLocaleString("fr-CA")} / ${target.toLocaleString("fr-CA")} kcal`
          : `Cible fixe ${target.toLocaleString("fr-CA")} kcal`;
        return `<div class="calorie-target-ring-card ${options.daily ? "is-daily" : ""} ${future ? "is-future" : ""}"><div class="calorie-target-ring" role="progressbar" aria-label="${esc(lead)} ${consumed} calories sur une cible fixe de ${target}" aria-valuemin="0" aria-valuemax="${target}" aria-valuenow="${Math.min(consumed, target)}"><svg class="calorie-target-ring-svg" viewBox="0 0 44 44" aria-hidden="true"><circle class="calorie-target-ring-outer-track" cx="22" cy="22" r="20" pathLength="100"></circle><circle class="calorie-target-ring-outer-progress" cx="22" cy="22" r="20" pathLength="100" style="--ring-pct:${pct}"></circle><circle class="calorie-target-ring-track" cx="22" cy="22" r="18" pathLength="100"></circle><circle class="calorie-target-ring-progress" cx="22" cy="22" r="18" pathLength="100" style="--ring-pct:${pct}"></circle></svg><div class="calorie-target-ring-center"><strong>${consumed.toLocaleString("fr-CA")}</strong><small>kcal</small></div></div><div class="calorie-target-ring-meta"><strong>${metaTitle}</strong><small>${esc(status)}${esc(partial)} · cible personnelle</small></div></div>`;
      }
      const amount = `${consumed.toLocaleString("fr-CA")} / ${target.toLocaleString("fr-CA")} kcal`;
      return `<div class="calorie-target-gauge"><div class="calorie-target-gauge-head"><span>Cible calorique fixe</span><strong>${amount}</strong></div><div class="calorie-target-gauge-track" role="progressbar" aria-label="${esc(lead)} ${consumed} calories sur une cible fixe de ${target}" aria-valuemin="0" aria-valuemax="${target}" aria-valuenow="${Math.min(consumed, target)}"><i style="width:${pct}%"></i></div><small>${esc(status)}${esc(partial)} · cible personnelle</small></div>`;
    }

    const targetData = estimated;
    // En mode calculé, le cercle complet représente la dépense quotidienne estimée.
  ...
  function renderProfile() {
    ...
    $$('[data-calorie-target-mode]').forEach((button) => {
      button.addEventListener("click", () => {
        if (!db.settings.calorieTargetGauge) return;
        const mode = button.dataset.calorieTargetMode === "fixed" ? "fixed" : "estimated";
        if (calorieTargetMode() === mode) return;
        db.settings.calorieTargetMode = mode;
        saveLocal("mode-cible-calorique");
        setDayChanged(todayKey());
        renderProfile();
      });
    });
    $("#settingFixedCalorieTarget")?.addEventListener("change", (event) => {
      const value = Number(event.target.value);
      db.settings.fixedCalorieTarget = Number.isFinite(value) && value > 0 ? Math.round(Math.min(10000, value)) : null;
      event.target.value = db.settings.fixedCalorieTarget || "";
      saveLocal("cible-calorique-fixe");
      setDayChanged(todayKey());
    });
  ...
  function updateMealDialogType(type) {
    ...
    // Les favoris et récents sont utiles aussi pour les collations.
    // Pour une collation, on garde la barre mais on masque seulement le raccourci
    // « repas précédent », qui est spécifique aux repas principaux.
    if (quickBar) quickBar.hidden = !(mainMeal || isSnack);
    if (previousButton) previousButton.hidden = isSnack;
  ...
  function updateMealEntryBrainGuide() {
    const beforeIcon = $("#beforeFeelingSummaryIcon"),
      ...
      beforeScores = picker?.children.length
        ? collectScoredFeelingScores(picker, "before")
        : feelingScoresFor(savedMeal, "before"),
      beforeDone = Object.keys(beforeScores).length > 0,
      descriptionDone = !!String($("#mealDescription")?.value || "").trim();

    // Étape 1 : tant que le ressenti avant manque, le cerveau reste sur "Avant".
    if (beforeIcon) {
      beforeIcon.textContent = beforeDone ? "🌤️" : "🧠";
  ...
  function openMeal(id = null, presetType = null, forceReadOnly = false) {
    ...
    const eatingReasonDetails = $("#eatingReasonDetails");
    if (eatingReasonDetails) eatingReasonDetails.hidden = db.settings?.showEatingReasons === false;
    ...
    // Sur iPhone, la synchronisation peut avoir déjà remplacé la copie locale
    // par un chemin Supabase. Résoudre l’URL signée dès l’ouverture évite que
    // la photo semble avoir disparu lorsque l’utilisateur revient au repas.
    hydrateMealPhotoDraftUrls();
  }
  async function hydrateMealPhotoDraftUrls() {
    let changed = false;
    for (const photo of mealPhotoDrafts) {
      if (!photo?.path || photo.url || photo.local) continue;
      const url = await signedPhoto(photo.path);
      if (!url) continue;
      photo.url = url;
      changed = true;
    }
