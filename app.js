...
function historyNegativeFeelingHtml(stats){const renderSection=(items,label,emptyTitle,emptyText)=>{const count=items.reduce((sum,item)=>sum+item.count,0);if(!items.length)return `<div class="dashboard-feelings-section is-empty"><p class="eyebrow">${esc(label)}</p><h4>${esc(emptyTitle)}</h4><p class="muted small">${esc(emptyText)}</p></div>`;return `<div class="dashboard-feelings-section"><div class="dashboard-feelings-section-head"><div><p class="eyebrow">${esc(label)}</p><h4>Éléments signalés</h4></div><span class="muted small">${count} fois</span></div><div class="history-feelings-list">${items.slice(0,4).map(item=>`<details class="history-feeling-detail"><summary><span class="history-feeling-detail-title">${item.emoji} ${esc(item.label)}</span><b>${item.count}</b></summary><div class="history-feeling-detail-body">${item.occurrences.slice(0,2).map(occ=>`<article class="history-feeling-occurrence"><div class="history-feeling-occurrence-head"><strong>${esc(formatDate(occ.date))}</strong><span>${esc(occ.mealTime)} · ${esc(occ.mealType)}</span></div><p>${esc(occ.mealDescription)} · ${occ.rating}/5${occ.notes?` · ${esc(occ.notes)}`:''}</p>${occ.priorMeals.length?`<div class="history-feeling-context"><span>3 repas précédents</span>${occ.priorMeals.map(meal=>`<div class="history-feeling-context-item"><strong>${esc(meal.description)}</strong><small>${esc(meal.time)} · ${esc(t(meal.type))}</small></div>`).join('')}</div>`:`<p class="muted small">Pas assez de repas précédents sur cette journée.</p>`}</article>`).join('')}</div></details>`).join('')}</div></div>`};return `<section class="card history-feelings-card">${renderSection(stats.negative||[],"Observations négatives","Aucun signalement","Les symptômes enregistrés ici apparaîtront automatiquement ici.")}${renderSection(stats.positive||[],"Observations positives","Aucun signalement","Les états positifs enregistrés ici apparaîtront automatiquement ici.")}</section>`}
function dashboardNegativeFeelingHtml(stats){const renderSection=(items,label,emptyText)=>{const count=items.reduce((sum,item)=>sum+item.count,0);if(!items.length)return `<div class="dashboard-feelings-section is-empty"><p class="eyebrow">${esc(label)}</p><h4>Aucun signalement</h4><p class="muted small">${esc(emptyText)}</p></div>`;return `<div class="dashboard-feelings-section"><div class="dashboard-feelings-section-head"><div><p class="eyebrow">${esc(label)}</p><h4>Éléments signalés</h4></div><span class="muted small">${count} fois</span></div><div class="dashboard-feelings-list">${items.slice(0,4).map(item=>`<details class="history-feeling-detail"><summary><span class="history-feeling-detail-title">${item.emoji} ${esc(item.label)}</span><b>${item.count}</b></summary><div class="history-feeling-detail-body">${item.occurrences.slice(0,2).map(occ=>`<article class="history-feeling-occurrence"><div class="history-feeling-occurrence-head"><strong>${esc(formatDate(occ.date))}</strong><span>${esc(occ.mealTime)} · ${esc(occ.mealType)}</span></div><p>${esc(occ.mealDescription)} · ${occ.rating}/5${occ.notes?` · ${esc(occ.notes)}`:''}</p>${occ.priorMeals.length?`<div class="history-feeling-context"><span>3 repas précédents</span>${occ.priorMeals.map(meal=>`<div class="history-feeling-context-item">${esc(meal.description)} · ${esc(meal.time)}</div>`).join('')}</div>`:''}</article>`).join('')}</div></details>`).join('')}</div></div>`};return `<section class="card wide dashboard-feelings-card"><div class="dashboard-feelings-sections">${renderSection(stats.negative||[],"Observations négatives","Les symptômes enregistrés ici apparaîtront automatiquement ici.")}${renderSection(stats.positive||[],"Observations positives","Les états positifs enregistrés ici apparaîtront automatiquement ici.")}</div></section>`}
...
if("serviceWorker" in navigator){
  window.addEventListener("load", async () => {
    try {
      const reg = await navigator.serviceWorker.register("./sw.js?v=3.5.2");
      await reg.update();
