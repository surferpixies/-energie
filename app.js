(() => {
  ...
  function renderProfile() {
    ...
    $("#app .stack")?.insertAdjacentHTML(
      "beforeend",
      `<section class="card profile-creator-card" aria-label="Créateur de l’application"><img src="./surferpixies-signature.png?v=3.55.16" alt="Logo SurferPixies"><div><strong>Philippe Dumont</strong><span>Créateur d’Énergie</span><small>© 2026 · Tous droits réservés</small></div></section>`,
    );
    decorateSupplementIcons();
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", async () => {
      try {
        const reg = await navigator.serviceWorker.register("./sw.js?v=3.55.16");
        await reg.update();
