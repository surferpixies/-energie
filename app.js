...
function dateSeed(key=""){return [...String(key)].reduce((sum,ch)=>(sum*31+ch.charCodeAt(0))>>>0,2166136261)}
...
function seasonalDecoration(dateKey){
 ...
 const date=localDate(dateKey),year=date.getFullYear(),month=date.getMonth()+1,day=date.getDate(),pick=(icons,label)=>({icon:icons[dateSeed(dateKey)%icons.length],label});
 const easter=easterDate(year),diff=Math.round((date-easter)/86400000);
 if(month===1&&day===1)return pick(["🎉","✨","🥳"],"Jour de l’An");
 if(month===2&&day===14)return pick(["❤️","💕","💗"],"Saint-Valentin");
 if(Math.abs(diff)<=3)return pick(["🐰","🥚","🐣","🌷"],"Pâques");
 if(month===4&&day===22)return pick(["🌎","🌱","🌿"],"Jour de la Terre");
 if(month===6&&day===24)return pick(["⚜️","🎆","💙"],"Fête nationale du Québec");
 if(month===7&&day===1)return pick(["🍁","🇨🇦","🎆"],"Fête du Canada");
 if(month===10)return pick(["🎃","👻","🦇","🕸️","🍬"],"Ambiance d’Halloween");
 if(month===12&&day===31)return pick(["🎉","🥂","✨"],"Veille du Jour de l’An");
 if(month===12)return pick(["🎄","⭐","🎁","🔔","🕯️","❄️"],"Ambiance de Noël");
 if(month>=3&&month<=5)return pick(["🌸","🌷","🌼","🪻","🌱"],"Printemps");
 if(month>=6&&month<=8)return pick(["☀️","🌻","🍉","🍦","🌊","🏖️"],"Été");
 if(month>=9&&month<=11)return pick(["🍂","🍁","🌰","☕","🌾"],"Automne");
 return pick(["❄️","☃️","🧣","🧤","🌨️"],"Hiver");
}
function seasonalDecorationHtml(dateKey){const item=seasonalDecoration(dateKey);return item?`<span class="seasonal-day-icon" role="img" aria-label="${esc(item.label)}" title="${esc(item.label)}">${item.icon}</span>`:""}
...
const ACTIVITY_INTENSITY_LABELS={low:"Faible",moderate:"Modérée",high:"Élevée"};
function normalizeActivity(a={}){const type=a.type||a.activity_type||"Autre",minutes=Math.max(0,Number(a.minutes??a.duration_minutes)||0),intensity=["low","moderate","high"].includes(a.intensity)?a.intensity:"moderate";const estimatedRaw=a.estimatedCalories??a.estimated_calories;const estimatedCalories=Number.isFinite(Number(estimatedRaw))?Math.max(0,Math.round(Number(estimatedRaw))):estimateActivityCalories(type,minutes,intensity);const actualRaw=a.actualCalories??a.actual_calories??a.caloriesActual??null;const actualCalories=actualRaw===""||actualRaw==null?null:Math.max(0,Math.round(Number(actualRaw)||0));return{id:a.id||uid(),type,minutes,intensity,estimatedCalories,actualCalories,at:a.at||a.recorded_at||new Date().toISOString()}}
function activityToCloud(a={}){const x=normalizeActivity(a);return{id:x.id,type:x.type,minutes:x.minutes,intensity:x.intensity,estimatedCalories:x.estimatedCalories,actualCalories:x.actualCalories,at:x.at}}
function estimateActivityCalories(type,minutes,intensity="moderate"){const rates=ACTIVITY_KCAL_PER_MIN[type]||ACTIVITY_KCAL_PER_MIN.Autre;return Math.max(0,Math.round((Number(minutes)||0)*(rates[intensity]||rates.moderate)))}
function activityCalories(a){return a.actualCalories!=null?Number(a.actualCalories)||0:Number(a.estimatedCalories)||0}
function activitySummary(day){const items=(day.activities||[]).map(normalizeActivity),minutes=items.reduce((sum,a)=>sum+(Number(a.minutes)||0),0),calories=items.reduce((sum,a)=>sum+activityCalories(a),0);return{minutes,calories,label:minutes?`${minutes} min`:'À noter',count:items.length}}
const SLEEP_MARKERS=[{id:"frequent-wakings",icon:"🔄",label:"Réveils fréquents"},{id:"bathroom",icon:"🚽",label:"Levé pour uriner"},{id:"apnea",icon:"😮‍💨",label:"Ronflements / apnée"},{id:"falling-asleep",icon:"😴",label:"Difficulté à s’endormir"},{id:"early-waking",icon:"⏰",label:"Réveil très tôt"},{id:"nightmares",icon:"😰",label:"Cauchemars"},{id:"discomfort",icon:"🥵",label:"Trop chaud / inconfort"},{id:"none",icon:"✅",label:"Rien de particulier"}];
function sleepMarker(id){return SLEEP_MARKERS.find(x=>x.id===id)}
...
function normalizeSupplements(value){return [...new Set((Array.isArray(value)?value:[value]).flatMap(v=>String(v||"").split(",")).map(x=>x.trim()).filter(Boolean))]}
const OBSERVATION_DURATIONS={ongoing:"Toujours présente",under_hour:"Moins d’une heure",few_hours:"Quelques heures",day:"Environ une journée",several_days:"Plusieurs jours",unknown:"Durée inconnue"};
const OBSERVATION_CONTEXTS={food:"Alimentation",sleep:"Sommeil",stress:"Stress",activity:"Activité",environment:"Environnement",unknown:"Inconnu"};
function normalObservation(o={},date=todayKey()){return{id:o.id||uid(),date:o.date||date,time:(o.time||o.startedAt||new Date().toTimeString().slice(0,5)).slice(0,5),intensity:Math.min(5,Math.max(1,Number(o.intensity)||3)),duration:OBSERVATION_DURATIONS[o.duration]?o.duration:"unknown",tags:Array.isArray(o.tags)?o.tags:[],contexts:Array.isArray(o.contexts)?o.contexts:[],mealIds:Array.isArray(o.mealIds)?o.mealIds:[],notes:typeof o.notes==="string"?o.notes:"",photoLocal:o.photoLocal||null,createdAt:o.createdAt||new Date().toISOString(),updatedAt:o.updatedAt||new Date().toISOString()}}
function observationForCloud(o){const clean=normalObservation(o,o.date);delete clean.photoLocal;return clean}
function freshDB(){return{version:CURRENT_VERSION,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),settings:{waterGoal:8,theme:"system",showWelcome:true,insightsEnabled:true,nutritionObservations:true,macroTracking:true,generalRecommendations:true,showSources:true,professionalSupport:false,feelingReminders:true,feelingDelayHours:2,feelingMealTypes:["Déjeuner","Dîner","Souper"],supplements:[],demoMode:false,demoTourSeen:false,demoName:"Marie",demoProfileId:"marie",demoReadOnly:true,seasonalIcons:true},favorites:[],days:{}}}
function ensureDay(store,key=todayKey()){if(!store.days[key])store.days[key]={date:key,sleepHours:null,sleepTags:[],sleepComment:"",water:0,activities:[],meals:[],observations:[],supplementsTaken:[],updatedAt:new Date().toISOString()};const d=store.days[key];d.activities=(Array.isArray(d.activities)?d.activities:[]).map(normalizeActivity);d.meals=Array.isArray(d.meals)?d.meals:[];d.observations=(Array.isArray(d.observations)?d.observations:[]).map(o=>normalObservation(o,key));d.sleepTags=Array.isArray(d.sleepTags)?d.sleepTags:[];d.sleepComment=typeof d.sleepComment==="string"?d.sleepComment:"";d.water=Number(d.water)||0;const defaults=normalizeSupplements(store.settings?.supplements||[]);d.supplementsTaken=normalizeSupplements(d.supplementsTaken||d.supplements||[]);if(!d.supplementsTaken.length&&defaults.length)d.supplementsTaken=[...defaults];return d}
function normalNutrition(n){if(!n||typeof n!=="object")return null;const val=k=>{const x=Number(n[k]);return Number.isFinite(x)&&x>=0?Math.round(x*10)/10:null};const out={calories:val("calories"),protein:val("protein"),carbs:val("carbs"),fat:val("fat"),fiber:val("fiber"),sugars:val("sugars"),sodium:val("sodium"),source:n.source||"manual",confidence:n.confidence||"low",basis:n.basis||"portion courante",estimated:n.estimated!==false};return [out.calories,out.protein,out.carbs,out.fat,out.fiber,out.sugars,out.sodium].some(v=>v!==null)?out:null}
function normalMeal(m={},date=todayKey()){return{id:m.id||uid(),date:m.date||date,time:m.time||"12:00",type:m.type||m.mealType||m.typeRepas||"Repas",description:m.description||m.food||m.aliments||m.repas||m.details||"",fatigueBefore:clamp(m.fatigueBefore??m.fatigueAvant??m.before,0,5),fatigueAfter:clamp(m.fatigueAfter??m.fatigueApres??m.after??m.fatigue1h??m.after1h,0,5),notes:m.notes||"",nutrition:normalNutrition(m.nutrition||m.macros),photoUrl:m.photoUrl||null,photoPath:m.photoPath||null,photoLocal:m.photoLocal||m.photo||m.image||null,createdAt:m.createdAt||new Date().toISOString(),updatedAt:m.updatedAt||new Date().toISOString(),feeling:m.feeling||null,feelingNotifiedAt:m.feelingNotifiedAt||null,recommendation:m.recommendation&&typeof m.recommendation==="object"?{category:m.recommendation.category||"",message:m.recommendation.message||""}:null}}
...
function migrate(raw){const out=freshDB();if(!raw||typeof raw!=="object")return out;out.settings={...out.settings,...(raw.settings||{})};out.settings.supplements=normalizeSupplements(out.settings.supplements||raw.settings?.supplements||[]);if(Number(raw.version||0)<19&&raw.settings?.macroTracking===false)out.settings.macroTracking=true;out.favorites=(raw.favorites||raw.favoriteMeals||[]).map(normalFavorite);if(raw.days&&typeof raw.days==="object"){Object.entries(raw.days).forEach(([k,d])=>{const day=ensureDay(out,k);day.sleepHours=d.sleepHours??d.sleep??d.sommeil??null;day.sleepTags=Array.isArray(d.sleepTags)?d.sleepTags:(Array.isArray(d.sleep_tags)?d.sleep_tags:[]);day.sleepComment=d.sleepComment??d.sleep_comment??"";day.water=Number(d.water??d.waterGlasses??d.eau??0)||0;day.activities=(Array.isArray(d.activities)?d.activities:[]).map(normalizeActivity);day.meals=(d.meals||d.repas||[]).map(m=>normalMeal(m,k));day.observations=(d.observations||[]).map(o=>normalObservation(o,k));day.supplementsTaken=normalizeSupplements(d.supplementsTaken||d.supplements||[]);if(!day.supplementsTaken.length&&out.settings.supplements.length)day.supplementsTaken=[...out.settings.supplements];day.updatedAt=d.updatedAt||new Date().toISOString()});return out}const arr=[raw.meals,raw.repas,raw.entries,raw.history,raw.logs,Array.isArray(raw)?raw:null].find(Array.isArray);if(arr)arr.forEach(x=>{const m=normalMeal(x,x.date||x.day||todayKey());ensureDay(out,m.date).meals.push(m)});return out}
...
let db=load();
function saveLocal(reason="local"){const demoPersistenceAllowed=/demo|visite/i.test(String(reason));if(db.settings?.demoMode&&db.settings?.demoReadOnly&&!demoPersistenceAllowed)return;db.version=CURRENT_VERSION;db.updatedAt=new Date().toISOString();const txt=JSON.stringify(db);localStorage.setItem(APP_KEY,txt);localStorage.setItem(`${APP_KEY}_shadow`,txt)}
const MEMORY_CLOUD_TABLE="user_food_memory";
let memorySyncTimer=null,memorySyncBusy=false;
function brainMemoryState(){try{return window.Brain?.memory?.exportState?.()||null}catch(_){return null}}
function maxObjectCounts(a={},b={}){const out={...a};Object.entries(b||{}).forEach(([key,value])=>{out[key]=Math.max(Number(out[key])||0,Number(value)||0)});return out}
function mergeMemoryRecord(local=null,remote=null){if(!local)return remote;if(!remote)return local;const localTime=new Date(local.lastSeenAt||0).getTime(),remoteTime=new Date(remote.lastSeenAt||0).getTime(),latest=remoteTime>localTime?remote:local;return{...local,...latest,id:local.id||remote.id,label:latest.label||local.label||remote.label,normalizedLabel:latest.normalizedLabel||local.normalizedLabel||remote.normalizedLabel,aliases:[...new Set([...(local.aliases||[]),...(remote.aliases||[])])].slice(0,20),mealTypes:maxObjectCounts(local.mealTypes,remote.mealTypes),ingredients:maxObjectCounts(local.ingredients,remote.ingredients),occurrences:Math.max(Number(local.occurrences)||0,Number(remote.occurrences)||0),confidence:Math.max(Number(local.confidence)||0,Number(remote.confidence)||0),firstSeenAt:[local.firstSeenAt,remote.firstSeenAt].filter(Boolean).sort()[0]||new Date().toISOString(),lastSeenAt:[local.lastSeenAt,remote.lastSeenAt].filter(Boolean).sort().slice(-1)[0]||new Date().toISOString(),sourceMealIds:[...new Set([...(local.sourceMealIds||[]),...(remote.sourceMealIds||[])])].slice(-50),isForgotten:Boolean(latest.isForgotten),metadata:{...(local.metadata||{}),...(remote.metadata||{})}}}
function cloudRowToMemory(row={}){return{id:row.id,label:row.label,normalizedLabel:row.normalized_label,aliases:row.aliases||[],mealTypes:row.meal_types||{},ingredients:row.ingredients||{},occurrences:Number(row.occurrences)||0,confidence:Number(row.confidence)||0,firstSeenAt:row.first_seen_at,lastSeenAt:row.last_seen_at,sourceMealIds:row.source_meal_ids||[],isForgotten:Boolean(row.is_forgotten),metadata:{...(row.metadata||{}),cloudUpdatedAt:row.updated_at}}}
function memoryToCloudRow(memory={}){return{id:memory.id,user_id:session.user.id,label:memory.label,normalized_label:memory.normalizedLabel,aliases:memory.aliases||[],meal_types:memory.mealTypes||{},ingredients:memory.ingredients||{},occurrences:Number(memory.occurrences)||0,confidence:Number(memory.confidence)||0,first_seen_at:memory.firstSeenAt,last_seen_at:memory.lastSeenAt,source_meal_ids:memory.sourceMealIds||[],is_forgotten:Boolean(memory.isForgotten),memory_version:Number(window.EnergieBrainModules?.memory?.version)||1,metadata:memory.metadata||{},updated_at:new Date().toISOString()}}
function mergeMemoryStates(localState,remoteRows=[]){const base=localState&&typeof localState==="object"?localState:{version:1,settings:{},memories:[]};const merged=new Map();(base.memories||[]).forEach(memory=>merged.set(memory.id||memory.normalizedLabel,memory));remoteRows.map(cloudRowToMemory).forEach(remote=>{let key=remote.id;let local=merged.get(key);if(!local){const entry=[...merged.entries()].find(([,memory])=>memory.normalizedLabel&&memory.normalizedLabel===remote.normalizedLabel);if(entry){key=entry[0];local=entry[1]}}merged.set(key,mergeMemoryRecord(local,remote))});return{...base,memories:[...merged.values()],updatedAt:new Date().toISOString()}}
async function syncMemoryCloud(){if(memorySyncBusy||!client||!session||!navigator.onLine||!window.Brain?.memory)return true;memorySyncBusy=true;try{const state=brainMemoryState();const rows=(state?.memories||[]).map(memoryToCloudRow);if(!rows.length)return true;const {error}=await client.from(MEMORY_CLOUD_TABLE).upsert(rows,{onConflict:"id"});if(error)throw error;return true}catch(error){console.warn("Synchronisation de la mémoire alimentaire",error);return false}finally{memorySyncBusy=false}}
function rebuildBrainMemoryFromJournal(){
  if(!window.Brain?.replaceMemoryState||!window.Brain?.learnMeals)return null;
  window.Brain.replaceMemoryState({version:1,settings:{},memories:[],updatedAt:new Date().toISOString()});
  const meals=allMealsFromDB(db).filter(meal=>!String(meal.id||"").startsWith("demo-"));
  if(meals.length)window.Brain.learnMeals(meals);
  return brainMemoryState();
}
async function replaceMemoryCloudFromJournal(){
  if(!client||!session||!navigator.onLine||!window.Brain?.memory)return true;
  try{
    const state=rebuildBrainMemoryFromJournal();
    const {error:deleteError}=await client.from(MEMORY_CLOUD_TABLE).delete().eq("user_id",session.user.id);
    if(deleteError)throw deleteError;
    const rows=(state?.memories||[]).map(memoryToCloudRow);
    if(rows.length){
      const {error:upsertError}=await client.from(MEMORY_CLOUD_TABLE).upsert(rows,{onConflict:"id"});
      if(upsertError)throw upsertError;
    }
    return true;
  }catch(error){console.warn("Réparation de la mémoire alimentaire",error);return false}
}
...
function scheduleMemoryCloudSync(){clearTimeout(memorySyncTimer);memorySyncTimer=setTimeout(()=>{if(session&&navigator.onLine)syncMemoryCloud()},700)}
...
async function syncNow(){if(db.settings.demoMode||!client||!session||!navigator.onLine)return;syncState="syncing";updateSyncBadge();const remaining=[];for(const op of outbox()){try{if(op.kind==="day"){const d=ensureDay(db,op.date);const dayPayload={user_id:session.user.id,log_date:op.date,sleep_hours:d.sleepHours,sleep_tags:d.sleepTags||[],sleep_comment:d.sleepComment||null,water:d.water,activities:(d.activities||[]).map(activityToCloud),supplements:{taken:d.supplementsTaken||[],defaults:db.settings.supplements||[]},updated_at:d.updatedAt};let {error}=await client.from("daily_logs").upsert(dayPayload,{onConflict:"user_id,log_date"});if(error&&/(sleep_tags|sleep_comment|supplements|schema cache|column)/i.test(error.message||"")){delete dayPayload.sleep_tags;delete dayPayload.sleep_comment;delete dayPayload.supplements;({error}=await client.from("daily_logs").upsert(dayPayload,{onConflict:"user_id,log_date"}))}if(error)throw error}else if(op.kind==="meal"){const meal=ensureDay(db,op.date).meals.find(m=>m.id===op.id);if(!meal)continue;await uploadPhoto(meal);const payload={id:meal.id,user_id:session.user.id,meal_date:meal.date,meal_time:meal.time,meal_type:meal.type,description:meal.description,fatigue_before:meal.fatigueBefore,fatigue_after:meal.fatigueAfter,notes:meal.notes||null,photo_path:meal.photoPath||null,feeling:meal.feeling||null,feeling_notified_at:meal.feelingNotifiedAt||null,nutrition:meal.nutrition||null,recommendation:meal.recommendation||null,created_at:meal.createdAt,updated_at:meal.updatedAt};let {error}=await client.from("meals").upsert(payload);if(error&&/nutrition|recommendation|schema cache|column/i.test(error.message||"")){delete payload.nutrition;delete payload.recommendation;({error}=await client.from("meals").upsert(payload))}if(error)throw error}else if(op.kind==="favorite"){const f=db.favorites.find(x=>x.id===op.id);if(!f)continue;const {error}=await client.from("favorite_meals").upsert({id:f.id,user_id:session.user.id,name:f.name,meal_type:f.type,description:f.description,notes:f.notes||null,usage_count:f.usageCount||0,created_at:f.createdAt,updated_at:f.updatedAt});if(error)throw error}else if(op.kind==="deleteMeal"){const {error}=await client.from("meals").delete().eq("id",op.id).eq("user_id",session.user.id);if(error)throw error;if(op.photoPath)await client.storage.from("meal-photos").remove([op.photoPath])}else if(op.kind==="deleteFavorite"){const {error}=await client.from("favorite_meals").delete().eq("id",op.id).eq("user_id",session.user.id);if(error)throw error}}catch(e){console.error("sync",e);remaining.push(op)}}setOutbox(remaining);const memoryOk=await syncMemoryCloud();syncState=remaining.length||!memoryOk?"error":"online";updateSyncBadge();if(!remaining.length)await pullCloud(false)}
async function pullCloud(show=true){if(db.settings.demoMode||!client||!session||!navigator.onLine)return;if(show){syncState="syncing";updateSyncBadge()}const [dr,mr,fr]=await Promise.all([client.from("daily_logs").select("*").eq("user_id",session.user.id).order("log_date"),client.from("meals").select("*").eq("user_id",session.user.id).order("meal_date").order("meal_time"),client.from("favorite_meals").select("*").eq("user_id",session.user.id).order("usage_count",{ascending:false})]);if(dr.error||mr.error||fr.error){console.error(dr.error||mr.error||fr.error);syncState="error";updateSyncBadge();return}for(const r of dr.data||[]){const d=ensureDay(db,r.log_date);if(!d.updatedAt||new Date(r.updated_at)>=new Date(d.updatedAt)){d.sleepHours=r.sleep_hours;d.sleepTags=Array.isArray(r.sleep_tags)?r.sleep_tags:[];d.sleepComment=r.sleep_comment||"";d.water=r.water||0;d.activities=(r.activities||[]).map(normalizeActivity);d.updatedAt=r.updated_at}}for(const r of mr.data||[]){const d=ensureDay(db,r.meal_date);const remote=normalMeal({id:r.id,date:r.meal_date,time:(r.meal_time||"").slice(0,5),type:r.meal_type,description:r.description,fatigueBefore:r.fatigue_before,fatigueAfter:r.fatigue_after,notes:r.notes,photoPath:r.photo_path,feeling:r.feeling||null,feelingNotifiedAt:r.feeling_notified_at||null,nutrition:r.nutrition||null,recommendation:r.recommendation||null,createdAt:r.created_at,updatedAt:r.updated_at},r.meal_date);const i=d.meals.findIndex(x=>x.id===remote.id);if(i<0)d.meals.push(remote);else if(new Date(remote.updatedAt)>=new Date(d.meals[i].updatedAt||0))d.meals[i]={...d.meals[i],...remote}}for(const r of fr.data||[]){const remote=normalFavorite({id:r.id,name:r.name,type:r.meal_type,description:r.description,notes:r.notes,usageCount:r.usage_count,createdAt:r.created_at,updatedAt:r.updated_at});const i=db.favorites.findIndex(x=>x.id===remote.id);if(i<0)db.favorites.push(remote);else if(new Date(remote.updatedAt)>=new Date(db.favorites[i].updatedAt||0))db.favorites[i]=remote}saveLocal("retour-cloud");const memoryOk=await pullMemoryCloud();syncState=memoryOk?"online":"error";updateSyncBadge();render()}
async function seedCloudFromLocal(){if(!session)return;const ops=[];Object.entries(db.days).forEach(([date,d])=>{ops.push({kind:"day",date});d.meals.forEach(m=>ops.push({kind:"meal",id:m.id,date}))});db.favorites.forEach(f=>ops.push({kind:"favorite",id:f.id}));setOutbox(ops);await syncNow();await syncMemoryCloud()}

function normalizeSnackText(value){return String(value||"").toLocaleLowerCase("fr-CA").normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[’']/g," ").replace(/[^a-z0-9]+/g," ").replace(/\s+/g," ").trim()}function snackIconForText(text){const value=normalizeSnackText(text);const icons=[];const add=(regex,icon)=>{if(regex.test(value))icons.push(icon)};add(/\b(pomme|pommes|apple|apples)\b/,"🍎");add(/\b(kiwi|kiwis)\b/,"🥝");add(/\b(banane|bananes|banana|bananas)\b/,"🍌");add(/\b(orange|oranges|mandarine|mandarines|clementine|clementines)\b/,"🍊");add(/\b(fraise|fraises|strawberry|strawberries)\b/,"🍓");add(/\b(raisin|raisins|grape|grapes)\b/,"🍇");add(/\b(poire|poires|pear|pears)\b/,"🍐");add(/\b(peche|peches|peach|peaches|melon|melons|mangue|mango|ananas|pineapple|pastèque|pasteque)\b/,"🍑");add(/\b(fromage|fromages|cheese|cheeses|feta|mozzarella|cheddar|gruyere|gouda|parmesan)\b/,"🧀");add(/\b(chips|croustilles|croustille|tortilla|nachos|crisps)\b/,"🍟");add(/\b(barre|barres|barre tendre|barres tendres|granola|protein bar|cereal bar|snack bar)\b/,"🍫");add(/\b(yogourt|yogourts|yaourt|yaourts|yogurt|yogurts)\b/,"🥣");add(/\b(gateau|gateaux|cake|cakes|brownie|brownies|muffin|muffins|cookie|cookies|biscuit|biscuits|donut|donuts|beigne|beignes|pain d epices|pain depices|tarte|tartes|pie|pies)\b/,"🍰");add(/\b(hummus|houmous|dip|tahini)\b/,"🫙");add(/\b(popcorn|pop corn|maïs éclaté|mais eclate)\b/,"🍿");add(/\b(pita|pain pita|wrap|tortilla|tortillas|naan|lavash|burrito|quesadilla)\b/,"🌯");add(/\b(soupe|soupes|soup|soups)\b/,"🥣");add(/\b(salami|saucisson|pepperoni|prosciutto|jambon|ham|charcuterie|rillettes|rillette|bresaola|coppa)\b/,"🥩");add(/\b(cafe|café|coffee|espresso|latte|cappuccino|thé|the|tea)\b/,"☕");add(/\b(pancake|pancakes|crepe|crepes|crêpe|crêpes|waffle|waffles|blinis)\b/,"🥞");add(/\b(taco|tacos|burrito|burritos|quesadilla|quesadillas|enchilada|enchiladas)\b/,"🌯");add(/\b(sushi|sashimi|maki|nori|onigiri|ramen|udon|pho)\b/,"🍣");add(/\b(noix|noisette|amande|amandes|cacahuete|cacahuetes|peanut|peanuts|almond|almonds)\b/,"🥜");add(/\b(carotte|carottes|carrot|carrots|concombre|concombres|cucumber|cucumbers|celeri|celeris|celery|brocoli|brocolis|broccoli|tomate|tomates|tomato|tomatoes|salade|salads|legume|legumes|vegetable|vegetables|poivron|poivrons|pepper|peppers|radis|radish|radishes|haricot|haricots|beans|bean)\b/,"🥕");add(/\b(legume|legumes|vegetable|vegetables)\b/,"🥗");return icons.length?icons.join(""):"🧺";}function mealIcon(t,description=""){if(t==="Collation")return snackIconForText(description);return({"Déjeuner":"🍳","Dîner":"🥗","Souper":"🍲","Boisson":"🥤"})[t]||"🍽️"}
...
function formatCalendarDate(k){return new Intl.DateTimeFormat("fr-CA",{day:"numeric",month:"long",year:"numeric"}).format(new Date(`${k}T12:00:00`))}
...
function nutritionText(n){if(!n)return"";const pieces=[];if(n.calories!=null)pieces.push(`${Math.round(n.calories)} kcal`);if(n.protein!=null)pieces.push(`${Number(n.protein).toFixed(n.protein%1?1:0)} g prot.`);if(n.carbs!=null)pieces.push(`${Number(n.carbs).toFixed(n.carbs%1?1:0)} g gluc.`);if(n.fat!=null)pieces.push(`${Number(n.fat).toFixed(n.fat%1?1:0)} g lip.`);if(n.fiber!=null)pieces.push(`${Number(n.fiber).toFixed(n.fiber%1?1:0)} g fibres`);if(n.sugars!=null)pieces.push(`${Number(n.sugars).toFixed(n.sugars%1?1:0)} g sucres`);if(n.sodium!=null)pieces.push(`${Math.round(n.sodium)} mg sodium`);return pieces.join(" · ")}
const FOOD_MACROS=Array.isArray(window.EnergieBrain?.legacyFoods)?window.EnergieBrain.legacyFoods:(Array.isArray(window.ENERGIE_FOODS)?window.ENERGIE_FOODS:[]);
const FOOD_NUTRIENT_OVERRIDES={
 "thon":{fiber:0,sugars:0,sodium:320},"thon en conserve":{fiber:0,sugars:0,sodium:320},"concombre":{fiber:0.5,sugars:1.7,sodium:2},"tomate":{fiber:1.5,sugars:3.2,sodium:8},"feta":{fiber:0,sugars:1.2,sodium:430},"huile d olive":{fiber:0,sugars:0,sodium:0},"mayonnaise":{fiber:0,sugars:0.1,sodium:90},
 "pomme":{fiber:4.4,sugars:19,sodium:2},"banane":{fiber:3.1,sugars:14,sodium:1},"orange":{fiber:3.1,sugars:12,sodium:0},"fraise":{fiber:3,sugars:7,sodium:2},"bleuet":{fiber:3.6,sugars:15,sodium:2},"framboise":{fiber:8,sugars:5.5,sodium:1},"avocat":{fiber:10,sugars:1,sodium:10},
 "avoine":{fiber:4,sugars:1,sodium:2},"gruau":{fiber:4,sugars:1,sodium:2},"pain":{fiber:1.2,sugars:1.5,sodium:170},"riz":{fiber:0.6,sugars:0.1,sodium:2},"riz brun":{fiber:3.5,sugars:0.7,sodium:10},"pâtes":{fiber:2.5,sugars:1.2,sodium:1},"quinoa":{fiber:5.2,sugars:1.6,sodium:13},
 "oeuf":{fiber:0,sugars:0.4,sodium:70},"œuf":{fiber:0,sugars:0.4,sodium:70},"poulet":{fiber:0,sugars:0,sodium:120},"saumon":{fiber:0,sugars:0,sodium:80},"tofu":{fiber:2,sugars:1,sodium:15},"lentille":{fiber:15.6,sugars:3.6,sodium:4},"pois chiche":{fiber:12.5,sugars:8,sodium:20},
 "yogourt":{fiber:0,sugars:12,sodium:95},"yaourt":{fiber:0,sugars:12,sodium:95},"yogourt grec":{fiber:0,sugars:6,sodium:65},"lait":{fiber:0,sugars:12,sodium:105},"fromage":{fiber:0,sugars:0.2,sodium:180},
 "brocoli":{fiber:5,sugars:2.2,sodium:50},"carotte":{fiber:3.6,sugars:6,sodium:88},"épinard":{fiber:4.3,sugars:0.7,sodium:126},"poivron":{fiber:2.5,sugars:5,sodium:5},"salade":{fiber:2,sugars:3,sodium:60},
 "beurre d arachide":{fiber:2,sugars:3,sodium:150},"amande":{fiber:3.5,sugars:1.2,sodium:0},"noix":{fiber:2,sugars:1,sodium:1},"chia":{fiber:10,sugars:0,sodium:5},
 "pizza":{fiber:4,sugars:7,sodium:1200},"hamburger":{fiber:2,sugars:7,sodium:950},"poutine":{fiber:7,sugars:3,sodium:1600},"soupe aux légumes":{fiber:5,sugars:8,sodium:700},"salade de pâtes":{fiber:4,sugars:5,sodium:550}
};
function foodNutrients(food){const key=normalizeFoodText(food?.keys?.[0]||"");const extra=FOOD_NUTRIENT_OVERRIDES[key]||{};return {...food,...extra}}
...
function comparableFoodText(value){return normalizeFoodText(value).split(" ").map(word=>word.length>4?word.replace(/[sx]$/i,""):word).join(" ")}
const FOOD_CANDIDATES=FOOD_MACROS.flatMap((food,foodIndex)=>(food.keys||[]).map(key=>({food,foodIndex,key:normalizeFoodText(key),compareKey:comparableFoodText(key)})).filter(item=>item.key)).sort((a,b)=>b.compareKey.length-a.compareKey.length);
function foodMatchForSegment(segment){
 const clean=normalizeFoodText(segment),comparable=comparableFoodText(segment);if(!clean)return null;
 const padded=` ${comparable} `;let best=null;
 for(const candidate of FOOD_CANDIDATES){
  const key=candidate.compareKey;if(!padded.includes(` ${key} `))continue;
  const keyWords=key.split(" ").length,cleanWords=comparable.split(" ").length;
...
function splitMealIngredients(text){
 const explicit=String(text||"").split(/\s*(?:\+|,|;|\n|\r|\u2022|\|)\s*/).map(x=>x.trim()).filter(Boolean);
 const initial=explicit.length?explicit:[String(text||"").trim()].filter(Boolean);
 const output=[];
 for(const segment of initial){
  const whole=foodMatchForSegment(segment);
  const connectorParts=segment.split(/\s+(?:et|and|avec|with)\s+/i).map(x=>x.trim()).filter(Boolean);
  if(connectorParts.length>1){
   const partMatches=connectorParts.map(foodMatchForSegment);
   if(partMatches.every(Boolean)){output.push(...connectorParts);continue}
  }
...
function estimateNutritionFromText(text){
 const segments=splitMealIngredients(text);if(!segments.length)return null;
 const matched=segments.map(segment=>({segment,food:foodMatchForSegment(segment)})).filter(x=>x.food);
 if(!matched.length)return null;
 const enriched=matched.map(x=>({...x,food:foodNutrients(x.food)}));const sum=k=>enriched.every(x=>x.food[k]!=null)?Math.round(enriched.reduce((a,x)=>a+(Number(x.food[k])||0),0)*10)/10:null;const total={calories:sum("calories"),protein:sum("protein"),carbs:sum("carbs"),fat:sum("fat"),fiber:sum("fiber"),sugars:sum("sugars"),sodium:sum("sodium")};
 const portions=[...new Set(enriched.map(x=>x.food.portion).filter(Boolean))];
 const basis=matched.length===1?(portions[0]||"portion courante"):`${matched.length} ingrédients estimés`;
 return normalNutrition({...total,source:"energie-foods",confidence:matched.length>=2?"medium":"low",basis,estimated:true})
}
function mergeNutrition(a,b){a=normalNutrition(a);b=normalNutrition(b);if(!a)return b;if(!b)return a;const add=k=>a[k]!=null&&b[k]!=null?a[k]+b[k]:(a[k]??b[k]??null);return normalNutrition({calories:add("calories"),protein:add("protein"),carbs:add("carbs"),fat:add("fat"),fiber:add("fiber"),sugars:add("sugars"),sodium:add("sodium"),source:a.source===b.source?a.source:"mixed",confidence:(a.confidence==="low"||b.confidence==="low")?"low":"medium",basis:"éléments additionnés",estimated:a.estimated||b.estimated})}
function nutritionFromInputs(){const get=id=>{const value=$(id)?.value;if(value===""||value==null)return null;const n=Number(value);return Number.isFinite(n)&&n>=0?n:null};return normalNutrition({calories:get("#nutritionCalories"),protein:get("#nutritionProtein"),carbs:get("#nutritionCarbs"),fat:get("#nutritionFat"),fiber:get("#nutritionFiber"),sugars:get("#nutritionSugars"),sodium:get("#nutritionSodium"),source:$("#mealNutritionSection")?.dataset.source||"manual",confidence:$("#mealNutritionSection")?.dataset.confidence||"low",basis:$("#mealNutritionSection")?.dataset.basis||"portion courante",estimated:$("#mealNutritionSection")?.dataset.estimated!=="false"})}
function fillNutritionInputs(n,note=""){n=normalNutrition(n);[["#nutritionCalories","calories"],["#nutritionProtein","protein"],["#nutritionCarbs","carbs"],["#nutritionFat","fat"],["#nutritionFiber","fiber"],["#nutritionSugars","sugars"],["#nutritionSodium","sodium"]].forEach(([id,key])=>{$(id).value=n?.[key]??""});const section=$("#mealNutritionSection");if(section){section.dataset.source=n?.source||"manual";section.dataset.confidence=n?.confidence||"low";section.dataset.basis=n?.basis||"portion courante";section.dataset.estimated=String(n?.estimated!==false)}$("#nutritionEstimateNote").textContent=note||(n?.source==="barcode"?`Valeurs ${n.basis||"du produit"} provenant de l’étiquette Open Food Facts. Vérifie-les au besoin.`:"Estimation approximative basée sur une portion courante. Les recettes et portions réelles peuvent varier.")}
...
function recommendationMealProfile(meal){
 ...
 const hasTag=(...wanted)=>wanted.some(tag=>tagSet.includes(normalizeFoodText(tag)));
 const normalizedText=recommendationNormalize(text);
 const hasBeefProtein=/(boeuf|bœuf|viande hach|steak|hamburger|bifteck|sausage|saucisse)/i.test(normalizedText);
 const flags={
  protein:hasTag("proteine","protein")||recommendationHas(text,RECOMMENDATION_WORDS.protein)||(Number(nutrition?.protein)||0)>=12||hasBeefProtein,
  vegetables:hasTag("legume","legumes","vegetable","vegetables")||recommendationHas(text,RECOMMENDATION_WORDS.vegetables)||/\b(legumes?|vegetables?|brocolis?|concombres?|tomates?|celeris?|carottes?|poivrons?|epinards?|courgettes?|choux|asperges?|champignons?|avocats?|aubergines?|betteraves?|haricots? verts?)\b/i.test(normalizedText),
  fruit:hasTag("fruit")||recommendationHas(text,RECOMMENDATION_WORDS.fruit),
  fiber:hasTag("fibre","fiber","grain","grains","cereal","cereales","cereale")||recommendationHas(text,RECOMMENDATION_WORDS.fiber),
  sugary:hasTag("sucre","sugar","sucree","sucrée","sugary")||recommendationHas(text,RECOMMENDATION_WORDS.sugary),
...
function chooseMealRecommendation(date,justSavedMeal){
 ...
 if(moment==="dinner"){
  if(!saved.flags.protein)add("protein",english
  ...
  if(!saved.flags.vegetables)add("vegetables",english
...
function demoAnalysisContext(){if(!db.settings?.demoMode)return null;const dates=Object.keys(db.days||{}).sort();if(!dates.length)return null;const first=dates[0],last=dates[dates.length-1],cutoff=selectedDate<first?first:(selectedDate>last?last:selectedDate),elapsed=Math.max(1,dates.filter(d=>d<=cutoff).length),total=Math.max(1,dates.length);return{first,last,cutoff,elapsed,total,progress:Math.max(0,Math.min(1,elapsed/total))}}
function demoReferenceStory(profileId,progress,base){const stage=progress<.2?0:progress<.45?1:progress<.72?2:3;const stories={marie:[{strength:"Tu documentes régulièrement tes repas malgré tes horaires variables.",habit:"Les produits laitiers apparaissent encore dans plusieurs déjeuners, cafés et collations.",suggestion:"Continue à noter les maux de tête et le contexte des repas afin de voir si une répétition apparaît."},{strength:"Tu commences à préparer davantage de repas maison.",habit:"Les maux de tête reviennent plus souvent durant les périodes où les produits laitiers sont très présents.",suggestion:"Observe quelques semaines en réduisant graduellement certains produits laitiers, sans tout changer à la fois."},{strength:"Les alternatives sans lactose deviennent plus fréquentes et tes repas maison gagnent du terrain.",habit:"Les semaines avec moins de produits laitiers contiennent déjà moins de mentions de maux de tête.",suggestion:"Poursuis la transition actuelle pour vérifier si cette amélioration se répète."},base],alex:[{strength:"Tu tiens un journal assez régulier.",habit:"Tes repas sont déjà variés et structurés.",suggestion:"Continue à documenter les changements inhabituels afin de créer un bon point de comparaison."},{strength:"Tes routines d’activité et d’hydratation sont constantes.",habit:"Aucune association négative répétée ne se distingue jusqu’ici.",suggestion:"Continue normalement; le Cerveau vérifie surtout la stabilité de tes habitudes."},{strength:"Ton alimentation demeure variée et régulière.",habit:"Les journées actives sont souvent accompagnées d’un ressenti positif.",suggestion:"Aucune priorité particulière ne ressort; poursuis simplement ton suivi."},base],sophie:[{strength:"Tu as commencé à documenter tes inconforts digestifs.",habit:"L’hydratation et les aliments riches en fibres sont encore peu réguliers.",suggestion:"Augmente progressivement l’eau et les fibres afin d’observer comment ton confort évolue."},{strength:"Le gruau, les fruits et les légumes apparaissent plus souvent.",habit:"Les ballonnements demeurent présents, mais certaines journées mieux hydratées semblent plus confortables.",suggestion:"Continue les changements graduellement et garde une hydratation régulière."},{strength:"Ta nouvelle routine de fibres devient plus constante.",habit:"Les épisodes digestifs commencent à diminuer durant les semaines les mieux hydratées.",suggestion:"Maintiens cette progression afin de confirmer la tendance."},base]};return(stories[profileId]||[base,base,base,base])[stage]||base}
...
demoReferenceStory=function(profileId,progress,base){
  ...
  return [
    {strength:"Tu documentes régulièrement tes repas malgré tes horaires variables.",habit:"Les produits laitiers apparaissent encore dans plusieurs déjeuners, cafés et collations.",suggestion:"Continue à noter les ballonnements, gaz, crampes et autres signes digestifs ainsi que le contexte des repas."},
    {strength:"Tu commences à préparer davantage de repas maison.",habit:"Les inconforts digestifs reviennent plus souvent durant les périodes où les produits laitiers sont très présents.",suggestion:"Observe quelques semaines en réduisant graduellement certains produits laitiers, sans tout changer à la fois."},
    {strength:"Les alternatives sans lactose deviennent plus fréquentes et tes repas maison gagnent du terrain.",habit:"Les semaines avec moins de produits laitiers contiennent déjà moins de ballonnements, gaz et crampes.",suggestion:"Poursuis la transition actuelle pour vérifier si cette amélioration se répète."},
    base
  ][stage]||base;
};
function referenceFeelingStats(brain){
  if(!brain?.feelings)return null;
  const meals=allMealsFromDB(db).slice().sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`));
  const decorate=item=>{
    const occurrences=meals.filter(meal=>(meal.feeling?.tags||[]).includes(item.id)).slice(0,12).map(meal=>({date:meal.date,mealDescription:meal.description||"Repas",mealTime:meal.time||"12:00",mealType:t(meal.type),rating:meal.feeling?.rating||3,notes:meal.notes||meal.feeling?.notes||"",priorMeals:[]}));
    return{...item,occurrences:item.occurrences?.length?item.occurrences:occurrences}
  };
  const negative=(brain.feelings.negative||[]).map(decorate),positive=(brain.feelings.positive||[]).map(decorate);
  return{negative,positive,negativeCount:negative.reduce((sum,item)=>sum+(item.count||0),0),positiveCount:positive.reduce((sum,item)=>sum+(item.count||0),0)}
}
...
function bindDemoChrome(){
  ...
  $$(".why-demo-insight").forEach(button=>button.addEventListener("click",()=>{
    const details={
      lactose:["Produits laitiers et inconfort digestif","Sur 52 repas contenant des produits laitiers, 20 sont suivis de ballonnements, gaz, crampes, gargouillis, nausées ou selles molles. Sur les autres repas, ces signes apparaissent beaucoup moins souvent. L’application présente donc une association à surveiller, pas une intolérance confirmée."],
...
function installDemoReadOnlyGuard(){
  if(demoReadOnlyGuardInstalled)return;demoReadOnlyGuardInstalled=true;
  document.addEventListener("click",event=>{
    ...
    if(control.closest("[data-open-demo-profile],#leaveDemoProfile,#leaveDemoQuick,#replayDemoTour,.nav-item,.brain-proof,.why-demo-insight,#previousDay,#nextDay,#goToday,#analysisPreviousDay,#analysisNextDay,#analysisGoLatest,[data-global-observation],[data-quick-meal][data-edit-meal]"))return;
    if(control.matches("details,summary")||control.closest("details"))return;
    event.preventDefault();event.stopImmediatePropagation();
    alert("Ce profil de démonstration est en lecture seule.");
  },true);
  document.addEventListener("submit",event=>{
    if(!db.settings?.demoMode||!db.settings?.demoReadOnly)return;
    if(event.target.closest("#app")||event.target.matches("#mealForm,#sleepForm,#activityForm,#feelingForm,#globalObservationForm")){
      event.preventDefault();event.stopImmediatePropagation();alert("Ce profil de démonstration est en lecture seule.");
    }
  },true)
}
...
const demoTourSteps=[
  {view:"today",target:".meal-quick-grid",title:"Une journée, en un coup d’œil",text:"Phil voit immédiatement ses repas et peut compléter son journal en quelques secondes.",proof:"Les repas principaux et les collations restent accessibles sans transformer le journal en questionnaire."},
  ...
  {view:"insights",target:".demo-insight-card",title:"Chaque tendance montre ses preuves",text:"Le niveau de confiance, les groupes comparés et l’explication sont visibles directement dans la carte.",proof:"Par exemple : 38 % d’inconforts digestifs avec produits laitiers, contre 11 % sans eux dans cette démo."},
...
function journalBrainDailyMessage(dateKey){
 const entries=Object.entries(db.days||{}).filter(([date,day])=>date<=dateKey&&(day?.meals||[]).length);
 const days=entries.length,meals=entries.reduce((sum,[,day])=>sum+(day.meals||[]).length,0);
 const stage=journalBrainStage(days),previousDate=addDaysKey(dateKey,-1),previousMeals=[...(db.days?.[previousDate]?.meals||[])].sort((a,b)=>(b.time||"").localeCompare(a.time||""));
 const tagged=previousMeals.filter(meal=>meal.feeling?.tags?.length);
 const negative=tagged.map(meal=>({meal,tag:(meal.feeling.tags||[]).map(id=>FEELING_TAGS.find(item=>item.id===id)).find(item=>item?.group==="symptom")})).find(item=>item.tag);
 const positive=tagged.map(meal=>({meal,tag:(meal.feeling.tags||[]).map(id=>FEELING_TAGS.find(item=>item.id===id)).find(item=>item?.group==="positive")})).find(item=>item.tag);
 let eyebrow="Le cerveau apprend",title="Chaque journée nourrit ton arbre",text="Continue simplement à noter tes repas et ton ressenti. Je compare progressivement les journées qui se ressemblent.";
 if(negative){
  ...
  text="Le cerveau en prend note et vérifiera si cette situation se répète avant de te présenter une tendance.";
 }else if(positive){
  eyebrow="Une information utile de plus";
  title=`Hier, tu as noté « ${positive.tag.label} » après ${positive.meal.type.toLowerCase()}`;
  text="Cette observation positive nourrit aussi l’analyse et aide le cerveau à reconnaître les journées où tu te sens mieux.";
 }else if(days===0){
  eyebrow="Le cerveau commence avec toi";
  title="Ta première journée fera naître la pousse";
  text="Un repas, un niveau d’énergie ou un ressenti suffit pour commencer à construire ta mémoire personnelle.";
 }else{
  const messages=[
   ["Analyse en cours","Je compare maintenant tes repas avec ton énergie, ton sommeil et ton hydratation."],
   ["Ta mémoire grandit","Chaque nouvelle journée m’aide à distinguer une vraie répétition d’une simple coïncidence."],
   ["Le cerveau reste prudent","Je préfère attendre suffisamment de données plutôt que de te montrer une conclusion fragile."],
   ["Une journée de plus compte","Même une journée sans symptôme aide à créer un meilleur point de comparaison."]
  ];
  const seed=[...dateKey].reduce((sum,char)=>sum+char.charCodeAt(0),0)%messages.length;
  eyebrow=messages[seed][0];title=messages[seed][1];text=`${meals} repas et ${days} journée${days!==1?"s":""} alimentent maintenant ton arbre de connaissances.`;
 }
 const previous=days<25?0:days<50?25:days<100?50:100;
 const progress=stage.next?Math.max(4,Math.min(100,Math.round((days-previous)/(stage.next-previous)*100))):100;
...
function render(){if(selectedDate>todayKey())selectedDate=todayKey();document.documentElement.dataset.theme=db.settings.theme==="dark"?"dark":"";updateLivingHeader();$("#todayLabel").textContent=currentView==="today"?formatDate(selectedDate):formatDate(todayKey());$$('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.view===currentView));updateSyncBadge();({today:renderToday,history:renderHistory,insights:renderInsights,brain:renderBrain,profile:renderProfile}[currentView]||renderToday)();renderDemoChrome();bindViewSwipe()}
function mealCard(m,opts={}){const feeling=m.feeling;const feelingEligible=isFeelingEligible(m);const feelingPreview=feelingEligible?`<div class="meal-feeling-preview ${feeling?'is-set':'is-empty'}">${feeling?`<span>${feelingEmoji(feeling.rating)} ${feeling.rating}/5</span>${feeling.tags?.length?`<small>${feeling.tags.length} étiquette${feeling.tags.length>1?'s':''}</small>`:''}`:`<span>Ressenti</span><small>À noter</small>`}</div>`:"";return `<article class="card meal-card" data-meal="${m.id}" data-date="${m.date}"><div class="meal-thumb">${m.photoUrl||m.photoLocal?`<img src="${esc(m.photoUrl||m.photoLocal)}" alt="">`:mealIcon(m.type,m.description)}</div><div class="meal-card-body"><h3>${esc(m.description)}</h3><div class="meal-meta">${esc(m.time)} · ${esc(t(m.type))}${opts.showDate?` · ${esc(formatDate(m.date))}`:""}</div>${db.settings.macroTracking&&m.nutrition?`<div class="meal-macros">≈ ${esc(nutritionText(m.nutrition))}</div>`:""}${feelingPreview}<div class="meal-footer"><span class="chip">⚡ ${m.fatigueBefore||'—'}/5</span>${feelingEligible?`<button class="meal-feeling-inline ${feeling?'is-set':'is-empty'}" data-feeling="${m.id}" title="${feeling?'Modifier le ressenti':'Ajouter un ressenti'}">${feeling?`${feelingEmoji(feeling.rating)} ${feeling.rating}/5`:'Ressenti'}</button>`:""}</div></div><div class="meal-actions">${feelingEligible?`<button class="feeling-meal" data-feeling="${m.id}" title="${feeling?'Modifier le ressenti':'Ajouter un ressenti'}">${feeling?'😊':'＋😊'}</button>`:""}<button class="favorite-meal" data-favorite="${m.id}" title="Ajouter aux favoris">☆</button><button class="delete-meal" data-delete="${m.id}" title="Supprimer">×</button></div></article>`}
...
function changeJournalDay(offset){const nextDate=addDaysKey(selectedDate,offset);if(db.settings?.demoMode){const c=demoAnalysisContext();if(c&&(nextDate<c.first||nextDate>c.last))return}else if(nextDate>todayKey())return;selectedDate=nextDate;render()}
...
function openFeeling(id){
  ...
  $("#feelingTags").innerHTML=FEELING_CATEGORIES.map(category=>{
    ...
    const alwaysClosed=["positive","digestion"].includes(category.id);
    const open=!alwaysClosed&&(category.open||hasSelected);
...
function dailyMacroSummaryHtml(meals){
  ...
  const known=[];
  meals.forEach(meal=>{const saved=normalNutrition(meal.nutrition),estimated=estimateNutritionFromText(meal.description||"");const nutrition=saved?normalNutrition({...estimated,...saved,fiber:saved.fiber??estimated?.fiber,sugars:saved.sugars??estimated?.sugars,sodium:saved.sodium??estimated?.sodium}):estimated;if(nutrition)known.push(nutrition)});
  if(!known.length)return "";
  const sum=key=>Math.round(known.reduce((total,n)=>total+(Number(n[key])||0),0)*10)/10;
  const calories=Math.round(sum("calories"));
  const protein=sum("protein");
  const carbs=sum("carbs");
  const fat=sum("fat");
  const coverage=known.length===meals.length?"Tous les repas enregistrés ont été inclus.":`${known.length} entrée${known.length>1?'s':''} sur ${meals.length} ${known.length>1?'ont':'a'} pu être estimée${known.length>1?'s':''}.`;
  const extra=[{key:"fiber",icon:"🌾",label:"Fibres",unit:"g"},{key:"sugars",icon:"🍬",label:"Sucres",unit:"g"},{key:"sodium",icon:"🧂",label:"Sodium",unit:"mg"}].filter(x=>known.every(m=>m[x.key]!=null)).map(x=>{const value=Math.round(sum(x.key)*10)/10;return `<span><b>${x.icon} ${value.toLocaleString('fr-CA')} ${x.unit}</b><small>${x.label}</small></span>`}).join("");
...
function observationSectionHtml(day){const observations=[...(day?.observations||[])].sort((a,b)=>b.time.localeCompare(a.time));const cards=observations.map(o=>{const tags=(o.tags||[]).map(id=>FEELING_TAGS.find(x=>x.id===id)).filter(Boolean),lead=tags[0],more=Math.max(0,tags.length-1),contexts=(o.contexts||[]).map(id=>OBSERVATION_CONTEXTS[id]).filter(Boolean);return `<button type="button" class="global-observation-item" data-global-observation="${o.id}"><span class="global-observation-item-icon">${lead?.emoji||'🔎'}</span><span><strong>${lead?esc(t(lead.label)):'Observation'}${more?` +${more}`:''}</strong><small>${esc(o.time)} · Intensité ${o.intensity}/5 · ${esc(OBSERVATION_DURATIONS[o.duration]||'Durée inconnue')}</small>${contexts.length?`<em>${esc(contexts.join(' · '))}</em>`:''}</span><b>›</b></button>`}).join('');return `<section class="card global-observations-card"><div class="row observation-section-head"><div><p class="eyebrow">Ressenti hors repas</p><h3>🔎 Observations globales</h3></div><button type="button" class="secondary small" id="addGlobalObservation">+ Ajouter</button></div><p class="muted small">Pour noter un symptôme qui peut apparaître plus tard ou durer plusieurs jours, sans l’attribuer automatiquement au dernier repas.</p><div class="observation-list">${cards||'<div class="observation-empty"><span>🌿</span><small>Aucune observation globale cette journée.</small></div>'}</div></section>`}
function supplementsTodayHtml(day){const observationHtml=observationSectionHtml(day),defaults=normalizeSupplements(db.settings?.supplements||[]);if(!defaults.length)return observationHtml;const taken=normalizeSupplements(day?.supplementsTaken||[]);const items=defaults.map(name=>`<label class="supplement-toggle"><input type="checkbox" data-supplement-name="${esc(name)}" ${taken.includes(name)?'checked':''}><span>${esc(name)}</span></label>`).join("");return `${observationHtml}<section class="card supplement-card"><div class="row"><div class="hydration-heading"><span>💊</span><h3>Suppléments</h3></div><strong>${taken.length}/${defaults.length}</strong></div><p class="muted small">Coche ce que tu as pris aujourd’hui. Les éléments restants restent décochés.</p><div class="supplement-list">${items}</div></section>`}
...
function existingTrendDays(keys){return keys.map(key=>db.days[key]).filter(Boolean)}
function trendAverage(values){const clean=values.map(Number).filter(Number.isFinite);return clean.length?clean.reduce((sum,value)=>sum+value,0)/clean.length:null}
function principalMealsForTrend(day){return (day?.meals||[]).filter(meal=>["Déjeuner","Dîner","Souper"].includes(meal.type))}
function isCompleteMealForTrend(meal){
 const profile=recommendationMealProfile(meal);
 const supportive=profile.flags.vegetables||profile.flags.fruit||profile.flags.fiber;
...
function feelingScoreForTrend(meal){
 ...
 const positiveTags=tags.filter(id=>FEELING_TAGS.find(tag=>tag.id===id)?.group==="positive").length;
 const symptomTags=tags.filter(id=>FEELING_TAGS.find(tag=>tag.id===id)?.group==="symptom").length;
 const rating=(Number(feeling.rating)-1)/4;
 const tagAdjustment=Math.max(-.25,Math.min(.25,(positiveTags-symptomTags)*.08));
 return Math.max(0,Math.min(1,rating+tagAdjustment))
}
function weeklyTrendPeriod(endDate,offset=0){
 const keys=trendDateKeys(endDate,7,offset),days=existingTrendDays(keys);
 const sleep=trendAverage(days.map(day=>day.sleepHours).filter(value=>value!=null));
 const water=trendAverage(days.map(day=>(Number(day.water)||0)*.5));
 const meals=days.flatMap(principalMealsForTrend);
 const energy=trendAverage(meals.map(meal=>meal.fatigueBefore).filter(value=>Number(value)>0));
 const complete=meals.length?meals.filter(isCompleteMealForTrend).length/meals.length:null;
 const feelings=days.flatMap(day=>(day.meals||[]).map(feelingScoreForTrend).filter(value=>value!=null));
 const feeling=trendAverage(feelings);
 return{
  keys,daysCount:days.length,sleep,water,energy,complete,feeling,
  counts:{sleep:days.filter(day=>day.sleepHours!=null).length,water:days.filter(day=>Number(day.water)>0).length,energy:meals.filter(meal=>Number(meal.fatigueBefore)>0).length,complete:meals.length,feeling:feelings.length}
 }
}
function trendMetric({key,icon,label,current,previous,minCurrent,minPrevious,threshold,format,unit=""}){
 const enough=current.count>=minCurrent&&previous.count>=minPrevious&&current.value!=null&&previous.value!=null;
 if(!enough)return{key,icon,label,state:"unknown",arrow:"—",short:"Données à venir",detail:"Continue à remplir ton journal pour faire ressortir cette tendance."};
 ...
 return{key,icon,label,state,arrow,short:direction===0?"Stable":`${signed}${format(delta)}${unit}`,detail:`${format(previous.value)}${unit} → ${format(current.value)}${unit}`}
}
function weeklyTrendSummaryHtml(endDate){
 const current=weeklyTrendPeriod(endDate,0),previous=weeklyTrendPeriod(endDate,7);
 const decimal=value=>Number(value).toLocaleString("fr-CA",{minimumFractionDigits:1,maximumFractionDigits:1});
 const integer=value=>Math.round(value).toLocaleString("fr-CA");
 const metrics=[
  trendMetric({key:"sleep",icon:"😴",label:"Sommeil",current:{value:current.sleep,count:current.counts.sleep},previous:{value:previous.sleep,count:previous.counts.sleep},minCurrent:3,minPrevious:3,threshold:.25,format:decimal,unit:" h"}),
  trendMetric({key:"water",icon:"💧",label:"Hydratation",current:{value:current.water,count:current.counts.water},previous:{value:previous.water,count:previous.counts.water},minCurrent:3,minPrevious:3,threshold:.2,format:decimal,unit:" L"}),
  trendMetric({key:"energy",icon:"⚡",label:"Énergie",current:{value:current.energy,count:current.counts.energy},previous:{value:previous.energy,count:previous.counts.energy},minCurrent:3,minPrevious:3,threshold:.2,format:decimal,unit:"/5"}),
  trendMetric({key:"complete",icon:"🥗",label:"Repas complets",current:{value:current.complete*100,count:current.counts.complete},previous:{value:previous.complete*100,count:previous.counts.complete},minCurrent:3,minPrevious:3,threshold:5,format:integer,unit:" %"}),
  trendMetric({key:"feeling",icon:"😊",label:"Ressenti",current:{value:current.feeling*100,count:current.counts.feeling},previous:{value:previous.feeling*100,count:previous.counts.feeling},minCurrent:3,minPrevious:3,threshold:5,format:integer,unit:" %"})
 ];
 ...
 let headline="Les tendances se préciseront avec quelques jours de données.";
 let tone="neutral";
 if(known.length>=2){
  if(improvements.length>declines.length){headline=`La semaine va dans une direction positive${improvements.length?`, surtout pour ${improvements.slice(0,2).map(item=>item.label.toLowerCase()).join(" et ")}`:""}.`;tone="positive"}
  else if(declines.length>improvements.length){headline=`Quelques éléments sont en baisse${declines.length?`, notamment ${declines.slice(0,2).map(item=>item.label.toLowerCase()).join(" et ")}`:""}.`;tone="watch"}
  else headline="La semaine est plutôt stable par rapport aux 7 jours précédents."
 }
 const rows=metrics.map(metric=>`<div class="weekly-trend-row ${metric.state}"><span class="weekly-trend-icon">${metric.icon}</span><span class="weekly-trend-label">${esc(metric.label)}</span><strong class="weekly-trend-change">${metric.arrow} ${esc(metric.short)}</strong><small>${esc(metric.detail)}</small></div>`).join("");
 return `<details class="weekly-trends card"><summary><span class="weekly-trends-title"><b>📈</b><span><small>Tendances</small><strong>Les 7 derniers jours</strong></span></span><span class="weekly-trends-glance">${metrics.filter(metric=>metric.state!=="unknown").slice(0,3).map(metric=>`<i class="${metric.state}" title="${esc(metric.label)}">${metric.arrow}</i>`).join("")||"<em>À venir</em>"}</span><span class="weekly-trends-chevron">›</span></summary><div class="weekly-trends-body"><p class="weekly-trends-headline ${tone}">${esc(headline)}</p><div class="weekly-trends-grid">${rows}</div><p class="weekly-trends-note">Comparaison avec les 7 jours précédents. Ces variations montrent des associations dans ton journal, pas des liens de cause à effet.</p></div></details>`
}

function renderToday(){const d=ensureDay(db,selectedDate),goal=db.settings.waterGoal||8,meals=[...d.meals].sort((a,b)=>a.time.localeCompare(b.time)),avgBefore=average(meals.map(m=>m.fatigueBefore)),activity=activitySummary(d);const water=Array.from({length:goal},(_,i)=>`<button class="drop ${i<d.water?'filled':''}" data-water="${i+1}">💧</button>`).join("");const dayLabel=relativeDayLabel(selectedDate);const demoRange=demoAnalysisContext();const latestDate=demoRange?.last||todayKey();const isToday=selectedDate===latestDate;const sleepPct=d.sleepHours==null?0:Math.min(100,Math.max(0,Number(d.sleepHours)/8*100));const activityChips=(d.activities||[]).slice(0,3).map(a=>`<span class="activity-chip">${activityIcon(a.type)} ${esc(a.type)}</span>`).join('');const sleepChips=(d.sleepTags||[]).filter(x=>x!=='none').slice(0,2).map(id=>{const marker=sleepMarker(id);return marker?`<span class="sleep-chip">${marker.icon} ${esc(marker.label)}</span>`:''}).join('');const sleepExtra=Math.max(0,(d.sleepTags||[]).filter(x=>x!=='none').length-2);$("#app").innerHTML=`${!navigator.onLine?'<div class="offline-banner">Tu es hors ligne. Les changements seront synchronisés plus tard.</div>':''}<div id="journalView"><section class="journal-date-nav"><button class="journal-arrow" id="previousDay" aria-label="Jour précédent">‹</button><button class="journal-date-main ${isToday?'is-today':''}" id="goToday"><span>${esc(dayLabel)}</span><strong class="journal-date-value"><span class="seasonal-day-icon-wrap">${seasonalDecorationHtml(selectedDate)}</span><span>${esc(formatCalendarDate(selectedDate))}</span></strong></button><button class="journal-arrow ${selectedDate>=latestDate?'is-disabled':''}" id="nextDay" aria-label="Jour suivant" ${selectedDate>=latestDate?'disabled aria-disabled="true"':''}>›</button></section>${journalBrainCardHtml(selectedDate)}${weeklyTrendSummaryHtml(selectedDate)}${dailyMacroSummaryHtml(meals)}<section class="meal-quick-grid">${mealQuickCard('Déjeuner','🍳',meals)}${mealQuickCard('Dîner','🥪',meals)}${mealQuickCard('Souper','🍝',meals)}${mealQuickCard('Collation',mealIcon('Collation',meals.find(m=>m.type==='Collation')?.description||''),meals)}</section><section class="wellbeing-detail-grid"><button class="card sleep-card edit-sleep"><div class="wellness-head"><span class="wellness-icon">😴</span><div><small>Sommeil</small><strong>${d.sleepHours!=null?`${d.sleepHours} h`:'À noter'}</strong></div><b>›</b></div><div class="sleep-bar"><i style="width:${sleepPct}%"></i></div>${sleepChips||d.sleepComment?`<div class="sleep-chip-row">${sleepChips}${sleepExtra?`<span class="sleep-chip">+${sleepExtra}</span>`:''}${d.sleepComment?`<span class="sleep-comment-preview">📝 ${esc(d.sleepComment)}</span>`:''}</div>`:''}</button><button class="card activity-card edit-activity"><div class="wellness-head"><span class="wellness-icon">${(d.activities||[])[0]?activityIcon(d.activities[0].type):'🚶'}</span><div><small>Activité</small><strong>${activity.label}</strong></div><b>›</b></div><div class="activity-chip-row">${activityChips||'<span class="muted small">Choisir une activité</span>'}</div></button></section><section class="card hydration-card"><div class="row"><div class="hydration-heading"><span>💧 <small>(500 ml)</small></span><h3>Hydratation</h3></div><strong>${d.water}/${goal}</strong></div><div class="water-row">${water}</div></section>${supplementsTodayHtml(d)}</div>`;$("#previousDay").onclick=()=>changeJournalDay(-1);if(!$("#nextDay").disabled)$("#nextDay").onclick=()=>changeJournalDay(1);$("#goToday").onclick=()=>{selectedDate=latestDate;render()};$("#openJournalBrain")?.addEventListener("click",()=>{currentView="brain";render()});$('.edit-sleep')?.addEventListener('click',openSleep);$('.edit-activity')?.addEventListener('click',openActivity);$$('[data-quick-meal]').forEach(b=>b.onclick=()=>{const type=b.dataset.quickMeal,edit=b.dataset.editMeal;if(type==='Collation'&&mealTypeSummary(meals,'Collation').length)openSnackManager();else if(edit)openMeal(edit);else openMeal(null,type)});$$('[data-water]').forEach(b=>b.onclick=()=>{d.water=Number(b.dataset.water)===d.water?d.water-1:Number(b.dataset.water);setDayChanged(selectedDate);render()});$$('[data-supplement-name]').forEach(input=>input.onchange=()=>{const day=ensureDay(db,selectedDate);const name=input.dataset.supplementName;const taken=normalizeSupplements(day.supplementsTaken||[]);day.supplementsTaken=input.checked?[...new Set([...taken,name])].filter(Boolean):taken.filter(x=>x!==name);setDayChanged(selectedDate);render()});$('#answerFeeling')?.addEventListener('click',e=>{selectedDate=e.currentTarget.dataset.date;openFeeling(e.currentTarget.dataset.id)});bindJournalSwipe()}
...
function miniStatsHtml(stats,scope='day',meals=[]){const mealLabel=scope==='day'?(()=>{const c=dayMealCounts(meals);return `🍽️ ${c.main}/3${c.snacks?` · 🍎 ${c.snacks} collation${c.snacks>1?'s':''}`:''}${c.extras?` · ＋${c.extras}`:''}`})():`🍽️ ${stats.count}`;return `<div class="timeline-stats"><span>${mealLabel}</span><span>⚡ ${stats.avg==null?'—':stats.avg.toFixed(1)+'/5'}</span>${stats.fav?`<span>⭐ ${stats.fav}</span>`:''}${scope==='day'&&stats.first?`<span>🕒 ${stats.first}${stats.last!==stats.first?'–'+stats.last:''}</span>`:''}</div>`}
function dayInsight(meals){if(!meals.length)return '';const avg=average(meals.map(m=>m.fatigueBefore)),c=dayMealCounts(meals);if(c.main===3)return `Les trois repas principaux sont documentés${c.snacks?`, avec ${c.snacks} collation${c.snacks>1?'s':''} en complément`:''}. Ce suivi régulier donnera davantage de contexte aux tendances futures.`;if(avg!=null&&avg<=2)return `L’énergie notée avant les repas est plutôt faible pour cette journée, selon les données enregistrées.`;if(avg!=null&&avg>=4)return `L’énergie notée avant les repas est plutôt élevée pour cette journée. Cela ne permet pas d’en déterminer la cause.`;return `Cette journée contribue progressivement à mieux décrire tes habitudes.`}
...
function summaryTiles(stats,scope='day',meals=[]){const dayCounts=scope==='day'?dayMealCounts(meals):null;const tiles=[scope==='day'?`<div><span>🍽️</span><strong>${dayCounts.main}/3</strong><small>repas principaux</small></div>`:`<div><span>🍽️</span><strong>${stats.count}</strong><small>repas</small></div>`,`<div><span>⚡</span><strong>${stats.avg==null?'—':stats.avg.toFixed(1)+'/5'}</strong><small>fatigue moyenne</small></div>`];if(scope==='day'&&dayCounts.snacks)tiles.push(`<div><span>🍎</span><strong>${dayCounts.snacks}</strong><small>collation${dayCounts.snacks>1?'s':''}</small></div>`);if(scope!=='day')tiles.push(`<div><span>📅</span><strong>${stats.days||0}</strong><small>jours suivis</small></div>`);if(stats.fav)tiles.push(`<div><span>⭐</span><strong>${stats.fav}</strong><small>favoris</small></div>`);if(stats.avgSleep!=null)tiles.push(`<div><span>😴</span><strong>${stats.avgSleep.toFixed(1)} h</strong><small>sommeil moyen</small></div>`);if(stats.water)tiles.push(`<div><span>💧</span><strong>${stats.water}</strong><small>verres notés</small></div>`);return `<div class="period-summary-grid">${tiles.join('')}</div>`}
function periodObservation(stats,scope){if(!stats.count)return'';if(scope==='month'&&stats.days>=14)return`Tu as documenté ${stats.days} journées ce mois-ci. Ce volume permet de dégager des tendances plus représentatives, sans établir de lien de cause à effet.`;if(scope==='week'&&stats.days>=5)return`Cette semaine contient ${stats.count} repas répartis sur ${stats.days} jours. La régularité du suivi améliore le contexte des observations.`;return`Ce résumé repose uniquement sur les données que tu as enregistrées.`}
function historyNegativeFeelingStats(meals,days=30){const cutoff=new Date();cutoff.setDate(cutoff.getDate()-days+1);cutoff.setHours(0,0,0,0);const tagMeta=Object.fromEntries(FEELING_TAGS.map(tag=>[tag.id,tag]));const byDate={};meals.forEach(meal=>{if(!meal?.date)return;(byDate[meal.date]??=[]).push(meal)});const groups={negative:{},positive:{}};meals.filter(meal=>meal.feeling&&Array.isArray(meal.feeling.tags)&&meal.feeling.tags.length).forEach(meal=>{const occurrenceDate=new Date(`${meal.date}T12:00:00`);if(occurrenceDate<cutoff)return;const dayMeals=(byDate[meal.date]||[]).slice().sort((a,b)=>a.time.localeCompare(b.time));const mealIndex=dayMeals.findIndex(x=>x.id===meal.id);const priorMeals=mealIndex>=0?dayMeals.slice(Math.max(0,mealIndex-3),mealIndex):[];meal.feeling.tags.forEach(tag=>{const meta=tagMeta[tag];if(!meta)return;const bucket=meta.group==='symptom'?groups.negative:groups.positive;const entry=bucket[tag]||{id:tag,group:meta.group,label:meta.label,emoji:meta.emoji,count:0,occurrences:[]};entry.count+=1;entry.occurrences.push({date:meal.date,mealDescription:meal.description||'Repas',mealTime:meal.time||'12:00',mealType:t(meal.type),rating:meal.feeling.rating,notes:meal.feeling.notes||'',priorMeals:priorMeals.map(({description,time,type})=>({description,time,type}))});bucket[tag]=entry})});const sortEntries=items=>Object.values(items).sort((a,b)=>b.count-a.count);const negative=sortEntries(groups.negative),positive=sortEntries(groups.positive);return{negative,positive,negativeCount:negative.reduce((sum,item)=>sum+item.count,0),positiveCount:positive.reduce((sum,item)=>sum+item.count,0)}}
function feelingContextAverages(){const days=Object.values(db.days||{});const values={sleep:[],water:[],energy:[],activity:[],supplements:[]};const defaults=normalizeSupplements(db.settings?.supplements||[]);days.forEach(day=>{if(day.sleepHours!=null&&Number.isFinite(Number(day.sleepHours)))values.sleep.push(Number(day.sleepHours));if(Number(day.water)>0)values.water.push(Number(day.water)*.5);const energies=(day.meals||[]).map(meal=>Number(meal.fatigueBefore)).filter(value=>value>0);if(energies.length)values.energy.push(average(energies));const minutes=(day.activities||[]).reduce((sum,item)=>sum+(Number(item.minutes)||0),0);if(minutes>0)values.activity.push(minutes);if(defaults.length){const taken=normalizeSupplements(day.supplementsTaken||[]).filter(name=>defaults.includes(name)).length;values.supplements.push(taken/defaults.length)}});const result={};Object.entries(values).forEach(([key,list])=>{result[key]={value:trendAverage(list),count:list.length}});return result}
function feelingContextComparison(key,value,averageInfo){const averageValue=averageInfo?.value,samples=averageInfo?.count||0;if(value==null||!Number.isFinite(Number(value)))return{state:'unknown',label:'Non noté',detail:'Aucune donnée pour cette journée.'};if(samples<5||averageValue==null)return{state:'limited',label:'Données limitées',detail:'Encore quelques journées pour établir ton habitude.'};const difference=Math.abs(Number(value)-averageValue);const thresholds={sleep:[.55,1.35],water:[.45,1],energy:[.45,1.05],activity:[20,55],supplements:[.2,.5]};const [usual,very]=thresholds[key]||[Math.abs(averageValue)*.15,Math.abs(averageValue)*.35];const state=difference<=usual?'usual':difference<=very?'unusual':'very-unusual';return{state,label:state==='usual'?'Habituel':state==='unusual'?'Peu habituel':'Très inhabituel',detail:''}}
function feelingContextTile({icon,label,value,key,detail},averages){const comparison=feelingContextComparison(key,value,averages[key]);return `<div class="feeling-context-tile ${comparison.state}"><div class="feeling-context-tile-head"><span>${icon}</span><small>${esc(label)}</small></div><strong>${esc(detail||'—')}</strong><span class="feeling-context-badge">${esc(comparison.label)}</span></div>`}
function feelingOccurrenceContextHtml(occ,averages){const day=db.days?.[occ.date];const meals=day?.meals||[];const energies=meals.map(meal=>Number(meal.fatigueBefore)).filter(value=>value>0);const activityMinutes=(day?.activities||[]).reduce((sum,item)=>sum+(Number(item.minutes)||0),0);const defaults=normalizeSupplements(db.settings?.supplements||[]);const taken=normalizeSupplements(day?.supplementsTaken||[]).filter(name=>defaults.includes(name));const missing=defaults.filter(name=>!taken.includes(name));const tiles=[feelingContextTile({icon:'🌙',label:'Sommeil',value:day?.sleepHours!=null?Number(day.sleepHours):null,key:'sleep',detail:day?.sleepHours!=null?`${Number(day.sleepHours).toLocaleString('fr-CA',{maximumFractionDigits:1})} h`:'Non noté'},averages),feelingContextTile({icon:'💧',label:'Hydratation',value:Number(day?.water)>0?Number(day.water)*.5:null,key:'water',detail:Number(day?.water)>0?`${(Number(day.water)*.5).toLocaleString('fr-CA',{maximumFractionDigits:1})} L`:'Non notée'},averages),feelingContextTile({icon:'⚡',label:'Énergie',value:energies.length?average(energies):null,key:'energy',detail:energies.length?`${average(energies).toLocaleString('fr-CA',{maximumFractionDigits:1})}/5`:'Non notée'},averages),feelingContextTile({icon:'🚶',label:'Activité',value:activityMinutes>0?activityMinutes:null,key:'activity',detail:activityMinutes>0?`${Math.round(activityMinutes)} min`:'Non notée'},averages)];if(defaults.length)tiles.push(feelingContextTile({icon:'💊',label:'Suppléments',value:taken.length/defaults.length,key:'supplements',detail:missing.length?`${missing.length} non pris`:`${taken.length}/${defaults.length} pris`},averages));const mealsHtml=occ.priorMeals.length?`<div class="feeling-context-meals"><span>🍴 Repas précédents</span>${occ.priorMeals.map(meal=>`<div class="feeling-context-meal"><strong>${esc(meal.description)}</strong><small>${esc(meal.time)} · ${esc(t(meal.type))}</small></div>`).join('')}</div>`:`<p class="muted small feeling-context-no-meals">Aucun repas précédent enregistré cette journée.</p>`;return `<div class="feeling-day-context"><div class="feeling-context-title"><span>Contexte de cette journée</span><small>Comparé à tes habitudes personnelles</small></div><div class="feeling-context-tiles">${tiles.join('')}</div>${mealsHtml}</div>`}
function feelingObservationSectionHtml(items,label,emptyTitle,emptyText,limit=4,occurrenceLimit=2){const count=items.reduce((sum,item)=>sum+item.count,0),averages=feelingContextAverages();if(!items.length)return `<div class="dashboard-feelings-section is-empty"><p class="eyebrow">${esc(label)}</p><h4>${esc(emptyTitle)}</h4><p class="muted small">${esc(emptyText)}</p></div>`;return `<div class="dashboard-feelings-section"><div class="dashboard-feelings-section-head"><div><p class="eyebrow">${esc(label)}</p><h4>Éléments signalés</h4></div><span class="muted small">${count} fois</span></div><div class="history-feelings-list">${items.slice(0,limit).map(item=>`<details class="history-feeling-detail feeling-observation-detail"><summary><span class="history-feeling-detail-title">${item.emoji} ${esc(item.label)}</span><b>${item.count}</b></summary><div class="history-feeling-detail-body">${item.occurrences.slice(0,occurrenceLimit).map(occ=>`<article class="history-feeling-occurrence feeling-observation-occurrence"><div class="history-feeling-occurrence-head"><strong>${esc(formatDate(occ.date))}</strong><span>${esc(occ.mealTime)} · ${esc(occ.mealType)}</span></div><div class="feeling-occurrence-summary"><strong>${esc(occ.mealDescription)}</strong><span>Ressenti ${occ.rating}/5${occ.notes?` · ${esc(occ.notes)}`:''}</span></div>${feelingOccurrenceContextHtml(occ,averages)}</article>`).join('')}</div></details>`).join('')}</div></div>`}
function historyNegativeFeelingHtml(stats){return `<section class="card history-feelings-card">${feelingObservationSectionHtml(stats.negative||[],"Observations négatives","Aucun signalement","Les symptômes enregistrés ici apparaîtront automatiquement ici.",6,3)}${feelingObservationSectionHtml(stats.positive||[],"Observations positives","Aucun signalement","Les états positifs enregistrés ici apparaîtront automatiquement ici.",6,3)}</section>`}
function dashboardNegativeFeelingHtml(stats){return `<section class="card wide dashboard-feelings-card"><div class="dashboard-feelings-sections">${feelingObservationSectionHtml(stats.negative||[],"Observations négatives","Aucun signalement","Les symptômes enregistrés ici apparaîtront automatiquement ici.")}${feelingObservationSectionHtml(stats.positive||[],"Observations positives","Aucun signalement","Les états positifs enregistrés ici apparaîtront automatiquement ici.")}</div></section>`}
...
function foodTagsInText(value){
 const text=` ${normalizeFoodText(value)} `,tags=new Set();
 if(!text.trim())return tags;
 FOOD_MACROS.forEach(food=>{
  const found=(food.keys||[]).some(key=>{const normalized=normalizeFoodText(key);return normalized&&text.includes(` ${normalized} `)});
  if(found)(food.tags||[]).forEach(tag=>tags.add(normalizeFoodText(tag)));
 });
 return tags
}
function learnedHabitInsight(meals){
 const cutoff=new Date();cutoff.setDate(cutoff.getDate()-27);cutoff.setHours(0,0,0,0);
 const recent=meals.filter(m=>new Date(`${m.date}T12:00:00`)>=cutoff);
 if(recent.length<12)return null;
 const dates=[...new Set(recent.map(m=>m.date))],dayCount=dates.length;
 if(dayCount<8)return null;
 const hasTag=(meal,...wanted)=>{const tags=foodTagsInText(`${meal.description||""} ${meal.notes||""}`);return wanted.some(tag=>tags.has(normalizeFoodText(tag)))};
 const main=recent.filter(m=>["Déjeuner","Dîner","Souper"].includes(m.type));
 ...
 const vegetableDinners=dinners.filter(m=>hasTag(m,"légume","legume")).length;
 const proteinMeals=main.filter(m=>hasTag(m,"protéine","proteine")).length;
 const daysWithWater=dates.filter(date=>(ensureDay(db,date).water||0)>0);
 const waterGoal=Math.max(1,Number(db.settings.waterGoal)||8);
 const waterGoalDays=daysWithWater.filter(date=>(ensureDay(db,date).water||0)>=waterGoal).length;
 const candidates=[];
 const breakfastRatio=breakfastDays/dayCount;
 if(dayCount>=8&&breakfastRatio>=.6&&breakfastRatio<.95&&breakfasts.length>=4)candidates.push({score:breakfastRatio-.7,icon:"🥣",text:"Un déjeuner apparaît souvent dans les 28 derniers jours, ce qui peut refléter une routine du matin plutôt qu’un comportement inhabituel.",n:dayCount,basis:`Observation basée sur ${dayCount} jours des 28 derniers jours.`});
 if(snacks.length>=6&&fruitSnacks/snacks.length>=.5&&fruitSnacks/snacks.length<.95)candidates.push({score:fruitSnacks/snacks.length/10+.05,icon:"🍎",text:"Des collations fruitées apparaissent assez souvent dans ton historique récent.",n:snacks.length,basis:`Observation basée sur ${snacks.length} collations des 28 derniers jours.`});
 if(dinners.length>=5&&vegetableDinners/dinners.length>=.5&&vegetableDinners/dinners.length<.95)candidates.push({score:vegetableDinners/dinners.length/10+.04,icon:"🥦",text:"Les légumes apparaissent souvent dans tes soupers récents.",n:dinners.length,basis:`Observation basée sur ${dinners.length} soupers des 28 derniers jours.`});
 if(main.length>=8&&proteinMeals/main.length>=.6&&proteinMeals/main.length<.95)candidates.push({score:proteinMeals/main.length/10+.03,icon:"🍗",text:"Des sources de protéines apparaissent souvent dans tes repas principaux récents.",n:main.length,basis:`Observation basée sur ${main.length} repas principaux des 28 derniers jours.`});
 if(daysWithWater.length>=6&&waterGoalDays/daysWithWater.length>=.6&&waterGoalDays/daysWithWater.length<.95)candidates.push({score:waterGoalDays/daysWithWater.length/10+.02,icon:"💧",text:"Ton hydratation reste relativement régulière quand tu la documentes.",n:daysWithWater.length,basis:`Observation basée sur ${daysWithWater.length} jours d’hydratation des 28 derniers jours.`});
 const best=candidates.sort((a,b)=>b.score-a.score)[0];
 if(!best)return null;
 return{icon:best.icon,title:"Ce que j’apprends sur toi",text:best.text,confidence:insightConfidence(best.n),basis:best.basis,kind:"personal"}
}
...
function buildPersonalInsights(meals){const cards=[];if(!meals.length)return cards;const recent=meals.filter(m=>new Date(`${m.date}T23:59:59`)>=new Date(Date.now()-28*86400000));if(recent.length<8)return cards;const valid=recent.filter(m=>m.fatigueBefore>0);if(valid.length<6)return cards;const byType={};valid.forEach(m=>(byType[m.type]??=[]).push(m.fatigueBefore));const least=Object.entries(byType).filter(([,v])=>v.length>=3).map(([k,v])=>[k,average(v),v.length]).sort((a,b)=>b[1]-a[1])[0];if(least&&least[2]>=4){cards.push({icon:"🍽️",title:"Énergie observée avant certains repas",text:`Avant tes repas « ${least[0]} », ton énergie enregistrée est en moyenne de ${least[1].toFixed(1)}/5. Cela décrit ton historique sans expliquer la cause.`,confidence:insightConfidence(least[2]),basis:`Basé sur ${least[2]} repas enregistrés.`,kind:"personal"})}const learned=learnedHabitInsight(recent);if(learned)cards.push(learned);const weekdays={};valid.forEach(m=>{const day=new Date(`${m.date}T12:00:00`).toLocaleDateString('fr-CA',{weekday:'long'});(weekdays[day]??=[]).push(m.fatigueBefore)});const best=Object.entries(weekdays).filter(([,v])=>v.length>=3).map(([k,v])=>[k,average(v),v.length]).sort((a,b)=>b[1]-a[1])[0];if(best&&best[2]>=4){cards.push({icon:"📅",title:"Journée associée à plus d’énergie",text:`Le ${best[0]}, ton énergie avant les repas est en moyenne de ${best[1].toFixed(1)}/5 dans les données disponibles.`,confidence:insightConfidence(best[2]),basis:`Basé sur ${best[2]} repas enregistrés ce jour de la semaine.`,kind:"personal"})}return cards.slice(0,3)}
...
function dashboardNutritionSignals(meals){
  const groups=[
    {key:"protein",label:"les protéines",words:["oeuf","œuf","poulet","dinde","thon","saumon","poisson","tofu","tempeh","lentille","pois chiche","haricot","yogourt grec","fromage cottage","viande","boeuf","bœuf","porc","crevette"]},
    {key:"fibre",label:"les fibres",words:["avoine","gruau","pain complet","pain entier","riz brun","quinoa","lentille","pois chiche","haricot","chia","lin","son","céréale entière","cereale entiere"]},
    {key:"fruit",label:"les fruits",words:["pomme","banane","orange","clémentine","clementine","bleuet","fraise","framboise","raisin","mangue","ananas","poire","kiwi","melon","fruit"]},
    {key:"vegetable",label:"les légumes",words:["brocoli","salade","épinard","epinard","carotte","concombre","tomate","poivron","courgette","chou","asperge","haricot vert","légume","legume"]}
  ];
  const counts=Object.fromEntries(groups.map(g=>[g.key,0]));
  meals.forEach(meal=>{
    const text=`${meal.description||""} ${meal.notes||""}`.toLowerCase();
    groups.forEach(group=>{if(group.words.some(word=>text.includes(word)))counts[group.key]++});
  });
  const total=Math.max(1,meals.length);
  return groups.map(group=>({...group,count:counts[group.key],ratio:counts[group.key]/total}));
}
function dashboardStory(meals){
  const mealDates=meals.map(m=>m.date).filter(Boolean).sort();
  ...
  const strength=enough&&strongest.count>0
  ...
  const habit=enough&&priority.count<meals.length
...
function professionalDiscussionHtml(meals){
 ...
 const mains=meals.filter(m=>["Déjeuner","Dîner","Souper"].includes(m.type));
 const breakfasts=meals.filter(m=>m.type==="Déjeuner");
 const fibreWords=["fruit","pomme","poire","banane","baie","bleuet","framboise","légume","brocoli","carotte","salade","haricot","lentille","pois chiche","avoine","gruau","pain complet","blé entier","quinoa","noix","graine"];
 const proteinWords=["oeuf","œuf","yogourt","yaourt","fromage","lait","tofu","poulet","dinde","jambon","thon","saumon","poisson","viande","protéine","noix","beurre d'arachide","pois chiche"];
 const fibreMeals=mains.filter(m=>fibreWords.some(w=>text(m).includes(w))).length;
 const proteinBreakfasts=breakfasts.filter(m=>proteinWords.some(w=>text(m).includes(w))).length;
 if(mains.length>=8&&fibreMeals/mains.length<.35)subjects.push({icon:"🌾",title:"La place des fibres dans vos repas",text:"Les aliments riches en fibres apparaissent peu souvent dans les repas documentés."});
 if(breakfasts.length>=3&&proteinBreakfasts/breakfasts.length<.5)subjects.push({icon:"🥚",title:"Les protéines au déjeuner",text:"Plusieurs déjeuners enregistrés ne mentionnent pas clairement une source de protéines."});
 const lowAfternoon=meals.filter(m=>{const h=Number((m.time||"0:0").split(":")[0]);return h>=13&&h<=17&&Number(m.fatigueBefore||0)<=2}).length;
 if(lowAfternoon>=2)subjects.push({icon:"🌤️",title:"L’énergie en après-midi",text:"Une énergie plus faible a été notée à quelques reprises durant l’après-midi."});
 const byDay={};mains.forEach(m=>{(byDay[m.date]??=new Set()).add(m.type)});
 const trackedDays=Object.values(byDay);
 if(trackedDays.length>=5&&trackedDays.filter(x=>x.size<3).length/trackedDays.length>.5)subjects.push({icon:"🕒",title:"La régularité des repas",text:"Plusieurs journées ne contiennent pas les trois repas principaux dans le journal."});
 if(!subjects.length)subjects.push({icon:"💬",title:"Vos priorités actuelles",text:"Votre journal peut servir de point de départ pour préciser ce que vous souhaitez améliorer ou mieux comprendre."});
 return `<section class="card professional-discussion"><div class="professional-heading"><div class="professional-icon">👩‍⚕️</div><div><p class="eyebrow">Accompagnement professionnel</p><h3>À discuter avec votre professionnel</h3></div></div><p class="muted professional-intro">Voici des sujets possibles tirés uniquement de votre journal. Choisissez ceux qui vous semblent pertinents.</p><div class="professional-topics">${subjects.slice(0,3).map(x=>`<article><span>${x.icon}</span><div><strong>${esc(x.title)}</strong><p>${esc(x.text)}</p></div></article>`).join("")}</div><p class="professional-disclaimer">Ces observations servent à préparer une conversation. Elles ne constituent ni un diagnostic ni une recommandation médicale.</p></section>`;
}

const OBSERVATION_SEEN_KEY="energieObservationFirstSeenV1";
function observationFirstSeenMap(){try{return JSON.parse(localStorage.getItem(OBSERVATION_SEEN_KEY)||"{}")||{}}catch(_){return{}}}
function markAndDetectNewObservations(observations){
 const seen=observationFirstSeenMap(),now=Date.now(),sevenDays=7*86400000;
 let changed=false;
 const decorated=(observations||[]).map(observation=>{
  if(!seen[observation.id]){seen[observation.id]=new Date(now).toISOString();changed=true}
  const firstSeen=Date.parse(seen[observation.id]);
  return{...observation,isNew:Number.isFinite(firstSeen)&&now-firstSeen<=sevenDays}
 });
 if(changed)try{localStorage.setItem(OBSERVATION_SEEN_KEY,JSON.stringify(seen))}catch(_){}
 return decorated
}
function observationStrengthLabel(strength){return strength==="strong"?"Tendance forte":strength==="moderate"?"Tendance modérée":"Tendance légère"}
function brainGrowthState(maturity){
 ...
 if(days<11)return{plant:"🌱",title:"Je fais connaissance avec toi",text:"J’analyse progressivement tes repas et ton niveau d’énergie. "+phrase,next:25,label:"avant les premières tendances"};
 if(days<25)return{plant:"🌱",title:"Le cerveau d’Énergie apprend encore",text:"Je commence à voir certaines ressemblances, mais j’ai besoin d’un peu plus de données avant d’afficher une tendance. "+phrase,next:25,label:"avant les premières tendances"};
 if(days<50)return{plant:"🌿",title:"Premières connexions détectées",text:"J’ai maintenant assez de données pour commencer à repérer certaines associations dans ton journal. "+phrase,next:50,label:"pour renforcer les observations"};
 if(days<100)return{plant:"🌿",title:"Mes observations gagnent en confiance",text:"Les nouvelles journées renforcent progressivement certaines tendances et m’aident à écarter les coïncidences. "+phrase,next:100,label:"vers un journal riche"};
 return{plant:"🌳",title:"Ton journal est riche en données",text:"Ton historique permet maintenant de produire des observations beaucoup plus solides qu’au début. Je continue tout de même d’apprendre avec chaque nouvelle journée.",next:null,label:""}
}
function brainGrowthHeaderHtml(maturity){
 const days=Math.max(0,Number(maturity?.analyzableDays)||0),state=brainGrowthState(maturity);
 const previous=days<25?0:days<50?25:days<100?50:100;
 const progress=state.next?Math.max(0,Math.min(100,Math.round((days-previous)/(state.next-previous)*100))):100;
...
function discoverySectionHtml(report){
 const maturity=report?.maturity||{icon:"🌱",label:"Ton journal apprend encore",days:0,analyzableDays:0};
 const observations=markAndDetectNewObservations(report?.observations||[]);
 const mature=Number(maturity?.analyzableDays||0)>=100;
 const cards=observations.length?observations.map((x,i)=>`<article class="card observation-card discovery-card observation-${x.confidence.cls}">${x.isNew?`<div class="observation-new-badge">✨ Nouvelle tendance détectée</div>`:""}<div class="discovery-card-top"><div class="discovery-icon">${x.icon}</div><div class="observation-badges"><span class="observation-strength">${observationStrengthLabel(x.metrics?.strength)}</span><span class="discovery-level ${x.confidence.cls}">${x.confidence.icon} ${esc(x.confidence.label)}</span></div></div><h3>${esc(x.title)}</h3><p class="observation-text">${esc(x.text)}</p><div class="observation-comparison" aria-label="Comparaison de l’énergie moyenne"><div><span>Avec</span><strong>${esc(x.statistic)}/5</strong><small>${x.samples.exposed} journée${x.samples.exposed!==1?'s':''}</small></div><div class="observation-vs">vs</div><div><span>Sans</span><strong>${esc(x.comparisonStatistic)}/5</strong><small>${x.samples.comparison} journée${x.samples.comparison!==1?'s':''}</small></div></div><div class="discovery-meta"><span>${x.samples.total} journées analysées</span><button class="text-button why-discovery" data-discovery="${i}">Pourquoi je vois ceci?</button></div></article>`).join(''):`<section class="card discovery-empty observation-empty"><div class="food-art">${mature?'🔎':'🌱'}</div><h3>${mature?'Aucune association assez nette pour le moment':'Les premières tendances se préparent'}</h3><p>${mature?'Le journal contient beaucoup de données, mais aucune différence suffisamment claire et répétée ne ressort actuellement. Le Cerveau préfère ne pas créer une tendance artificielle.':'Continue simplement à remplir ton journal. Le cerveau d’Énergie compare déjà tes journées, mais préfère attendre avant de montrer une observation trop fragile.'}</p></section>`;
...
function parseServingGrams(value){const match=String(value||"").replace(",",".").match(/([0-9]+(?:\.[0-9]+)?)\s*g\b/i);const n=match?Number(match[1]):null;return Number.isFinite(n)&&n>0&&n<=2000?n:null}
function scaleNutritionPer100g(n,grams){n=normalNutrition(n);grams=Number(grams);if(!n||!Number.isFinite(grams)||grams<=0)return null;const f=grams/100,scale=k=>n[k]==null?null:Math.round(n[k]*f*10)/10;return normalNutrition({calories:scale("calories"),protein:scale("protein"),carbs:scale("carbs"),fat:scale("fat"),fiber:scale("fiber"),sugars:scale("sugars"),sodium:scale("sodium"),source:"barcode",confidence:"high",basis:`portion de ${Math.round(grams)} g`,estimated:true})}
function updateBarcodePortion(){if(!barcodeLastProduct)return;const grams=Number($("#barcodePortionGrams")?.value),base=barcodeLastProduct.nutritionPer100g;if(base&&Number.isFinite(grams)&&grams>0)barcodeLastProduct.nutrition=scaleNutritionPer100g(base,grams);const preview=$("#barcodeNutritionPreview");if(preview&&db.settings.macroTracking&&barcodeLastProduct.nutrition){preview.hidden=false;preview.innerHTML=`<strong>≈ ${esc(nutritionText(barcodeLastProduct.nutrition))}</strong><small>${esc(barcodeLastProduct.nutrition.basis||"portion indiquée")} · modifiable après l’ajout</small>`}}
function showBarcodeProduct(product,code,found=true){barcodeLastCode=code;barcodeLastProduct=product;const portionWrap=$("#barcodePortionWrap"),portionInput=$("#barcodePortionGrams");if(portionWrap&&portionInput){const suggested=product.servingGrams||100;portionInput.value=String(Math.round(suggested));portionWrap.hidden=!product.nutritionPer100g}const result=$("#barcodeResult"),name=$("#barcodeProductName"),brand=$("#barcodeBrand"),image=$("#barcodeProductImage"),badge=$("#barcodeResultBadge"),help=$("#barcodeResultHelp"),preview=$("#barcodeNutritionPreview");result.hidden=false;name.value=product.name||"";brand.textContent=product.brand||"";badge.textContent=found?"Produit reconnu":"Produit inconnu";help.textContent=found?"Tu peux simplifier ou modifier le nom avant de l’ajouter.":"Écris simplement le nom que tu veux ajouter au repas.";if(product.image){image.src=product.image;image.hidden=false}else{image.hidden=true;image.removeAttribute("src")}if(product.nutritionPer100g)updateBarcodePortion();if(db.settings.macroTracking&&product.nutrition){preview.hidden=false;preview.innerHTML=`<strong>≈ ${esc(nutritionText(product.nutrition))}</strong><small>${esc(product.nutrition.basis||"portion indiquée")} · modifiable après l’ajout</small>`}else preview.hidden=true;setTimeout(()=>name.focus(),80)}
async function lookupBarcode(code){code=cleanBarcode(code);if(code.length<6){alert("Entre un numéro de code-barres valide.");return}barcodeBusy=true;setBarcodeStatus("Recherche du produit…");const cached=barcodeCache()[code];if(cached?.name){showBarcodeProduct(cached,code,true);setBarcodeStatus("Produit trouvé dans tes scans récents.");barcodeBusy=false;stopBarcodeCamera();return}try{const url=`https://world.openfoodfacts.org/api/v2/product/${encodeURIComponent(code)}.json?fields=code,product_name,product_name_fr,product_name_en,brands,image_front_small_url,image_front_url,serving_size,nutriments`;const response=await fetch(url,{headers:{Accept:"application/json"}});if(!response.ok)throw new Error(`HTTP ${response.status}`);const data=await response.json(),p=data?.product;if(data?.status===1&&p){const perServing=p.nutriments||{},hasServing=[perServing["energy-kcal_serving"],perServing.proteins_serving,perServing.carbohydrates_serving,perServing.fat_serving,perServing.fiber_serving,perServing.sugars_serving,perServing.sodium_serving].some(v=>Number.isFinite(Number(v))),suffix=hasServing?"_serving":"_100g";const per100=normalNutrition({calories:perServing["energy-kcal_100g"],protein:perServing.proteins_100g,carbs:perServing.carbohydrates_100g,fat:perServing.fat_100g,fiber:perServing.fiber_100g,sugars:perServing.sugars_100g,sodium:Number(perServing.sodium_100g)*1000,source:"barcode",confidence:"high",basis:"pour 100 g",estimated:true}),servingGrams=parseServingGrams(p.serving_size)||100;const product={name:(p.product_name_fr||p.product_name||p.product_name_en||"").trim(),brand:(p.brands||"").split(",")[0].trim(),image:p.image_front_small_url||p.image_front_url||"",servingGrams,nutritionPer100g:per100,nutrition:per100?scaleNutritionPer100g(per100,servingGrams):normalNutrition({calories:perServing[`energy-kcal${suffix}`],protein:perServing[`proteins${suffix}`],carbs:perServing[`carbohydrates${suffix}`],fat:perServing[`fat${suffix}`],fiber:perServing[`fiber${suffix}`],sugars:perServing[`sugars${suffix}`],sodium:Number(perServing[`sodium${suffix}`])*1000,source:"barcode",confidence:hasServing?"high":"medium",basis:hasServing?(p.serving_size?`portion ${p.serving_size}`:"portion indiquée"):"pour 100 g",estimated:!hasServing})};if(product.name){saveBarcodeCache(code,product);showBarcodeProduct(product,code,true);setBarcodeStatus("Produit reconnu.")}else{showBarcodeProduct({},code,false);setBarcodeStatus("Le produit existe, mais son nom est manquant.")}}else{showBarcodeProduct({},code,false);setBarcodeStatus("Produit non trouvé. Tu peux entrer son nom.")}stopBarcodeCamera()}catch(e){console.warn("Open Food Facts",e);showBarcodeProduct({},code,false);setBarcodeStatus("Recherche impossible pour le moment. Entre le nom manuellement.");stopBarcodeCamera()}finally{barcodeBusy=false}}
...
function setMealSuggestion(recommendation,type){
 const wrap=$("#mealSuggestionWrap"),toggle=$("#mealSuggestionToggle"),body=$("#mealSuggestionBody");
 if(!wrap||!toggle||!body)return;
 const isMain=["Déjeuner","Dîner","Souper"].includes(type);
 const message=isMain&&recommendation?.message?String(recommendation.message).trim():"";
 wrap.hidden=!message;body.hidden=true;toggle.setAttribute("aria-expanded","false");toggle.textContent="💡 Voir la suggestion";body.textContent=message;
}
function toggleMealSuggestion(){
 const toggle=$("#mealSuggestionToggle"),body=$("#mealSuggestionBody");if(!toggle||!body)return;
 const open=body.hidden;body.hidden=!open;toggle.setAttribute("aria-expanded",String(open));toggle.textContent=open?"💡 Masquer la suggestion":"💡 Voir la suggestion";
}
function setDemoDetailReadOnly(formSelector,enabled){
  ...
  form.querySelectorAll("input,select,textarea,button").forEach(control=>{
    if(control.classList.contains("close-dialog")){control.disabled=false;return}
    if(enabled){
      if(!control.dataset.demoPreviousDisabled)control.dataset.demoPreviousDisabled=control.disabled?"1":"0";
...
function updateMealDialogType(type){const value=type||'Déjeuner';$("#mealType").value=value;$("#mealDialogTypeIcon").innerHTML=mealIcon(value);$("#mealDialogTypeLabel").textContent=t(value);$("#copyYesterdayBreakfast").hidden=value!=="Déjeuner";$("#copyYesterdayDinner").hidden=value!=="Dîner";populateFavoriteSelect(value);$("#favoriteMealSelect").value='';populateRecentFoods(value);if(!["Déjeuner","Dîner","Souper"].includes(value))setMealSuggestion(null,value)}function updateMealFeelingUi(meal){const button=$("#mealFeelingButton"),summary=$("#mealFeelingSummary");if(!button||!summary)return;const hasMeal=!!meal;button.hidden=!hasMeal;if(!hasMeal){summary.textContent='';return}const feeling=meal.feeling;const text=feeling?`${feelingEmoji(feeling.rating)} ${feeling.rating}/5${feeling.tags?.length?` · ${feeling.tags.length} étiquette${feeling.tags.length>1?'s':''}`:''}`:'Aucun ressenti';summary.textContent=text;button.textContent=feeling?'Modifier le ressenti':'Ajouter un ressenti';button.classList.toggle('is-set',!!feeling);button.classList.toggle('is-empty',!feeling);button.onclick=()=>{if(meal?.id)openFeeling(meal.id)}}function openMeal(id=null,presetType=null){const d=ensureDay(db,selectedDate),m=id?d.meals.find(x=>x.id===id):null,type=m?.type||presetType||'Déjeuner',readOnly=!!(db.settings?.demoMode&&db.settings?.demoReadOnly&&m);setDemoDetailReadOnly("#mealForm",false);$("#mealDialogTitle").textContent=readOnly?'Détails du repas':(m?'Modifier le repas':'Ajouter un repas');$("#mealId").value=m?.id||'';const deleteButton=$("#deleteCurrentMeal");deleteButton.hidden=!m;deleteButton.onclick=m?()=>{if(confirm(`Supprimer « ${m.description} »?`)){deleteMealLocal(m);$("#mealDialog").close();render()}}:null;updateMealDialogType(type);$("#mealTime").value=m?.time||new Date().toTimeString().slice(0,5);$("#mealDescription").value=m?.description||'';const editRecommendation=(m&&["Déjeuner","Dîner","Souper"].includes(type))?chooseMealRecommendation(selectedDate,m):null;setMealSuggestion(editRecommendation,type);$("#mealNotes").value=m?.notes||'';$("#mealNutritionSection").hidden=!db.settings.macroTracking;fillNutritionInputs(m?.nutrition?normalNutrition({...estimateNutritionFromText(m.description||""),...m.nutrition,fiber:m.nutrition.fiber??estimateNutritionFromText(m.description||"")?.fiber,sugars:m.nutrition.sugars??estimateNutritionFromText(m.description||"")?.sugars,sodium:m.nutrition.sodium??estimateNutritionFromText(m.description||"")?.sodium}):null);makeRatings('#fatigueBeforePicker',m?.fatigueBefore??3);updateMealFeelingUi(m);$("#mealFeelingButton").onclick=m?()=>openFeeling(m.id):null;photoData=m?.photoLocal||m?.photoUrl||null;photoRemoved=false;showPhotoPreview();setDemoDetailReadOnly("#mealForm",readOnly);$("#mealDialog").showModal()}
function showPhotoPreview(){const wrap=$("#photoPreviewWrap");wrap.hidden=!photoData;if(photoData)$("#photoPreview").src=photoData}
function renderDayActivities(){const d=ensureDay(db,selectedDate),list=$("#dayActivitiesList");if(!list)return;list.innerHTML=(d.activities||[]).length?(d.activities||[]).map(raw=>{const a=normalizeActivity(raw),kcal=activityCalories(a),source=a.actualCalories!=null?"mesurées":"estimées";return `<div class="saved-activity"><span>${activityIcon(a.type)}</span><div><strong>${esc(a.type)}</strong><small>${Number(a.minutes)||0} min · ${esc(ACTIVITY_INTENSITY_LABELS[a.intensity]||"Modérée")} · ${Math.round(kcal)} kcal ${source}</small></div><button type="button" data-delete-activity="${esc(a.id)}" aria-label="Supprimer ${esc(a.type)}">×</button></div>`}).join(''):'<p class="muted small">Aucune activité enregistrée pour cette journée.</p>';$$('[data-delete-activity]').forEach(b=>b.onclick=()=>{d.activities=d.activities.filter(a=>a.id!==b.dataset.deleteActivity);setDayChanged(selectedDate);renderDayActivities()})}
function updateActivityEstimate(){const type=$("#activityType")?.value||"Autre",minutes=parseAppNumber($("#activityMinutes")?.value)||0,intensity=$("#activityIntensity")?.value||"moderate",estimate=estimateActivityCalories(type,minutes,intensity),value=$("#activityEstimateValue");if(value)value.textContent=minutes>0?`${estimate} kcal`:"— kcal"}
function setActivityIntensity(value="moderate"){$("#activityIntensity").value=value;$$("[data-intensity]").forEach(b=>b.classList.toggle("active",b.dataset.intensity===value));updateActivityEstimate()}
function openSleep(){const d=ensureDay(db,selectedDate);$('#sleepHours').value=d.sleepHours??'';$('#sleepComment').value=d.sleepComment||'';$$('[data-sleep-tag]').forEach(input=>{input.checked=(d.sleepTags||[]).includes(input.value)});$('#sleepDialog').showModal()}
function openActivity(){ensureDay(db,selectedDate);$('#activityType').value='';$('#activityMinutes').value='';$('#activityActualCalories').value='';$$('[data-activity]').forEach(b=>b.classList.remove('active'));setActivityIntensity('moderate');renderDayActivities();updateActivityEstimate();$('#activityDialog').showModal()}
...
function findGlobalObservation(id){for(const [date,day] of Object.entries(db.days||{})){const observation=(day.observations||[]).find(o=>o.id===id);if(observation)return{date,day,observation}}return null}
function observationTagPickerHtml(selected){const chosen=new Set(selected||[]);return FEELING_CATEGORIES.map(category=>{const tags=FEELING_TAGS.filter(tag=>tag.category===category.id),alwaysClosed=["positive","digestion"].includes(category.id);const open=!alwaysClosed&&(category.open||tags.some(tag=>chosen.has(tag.id)));return `<details class="feeling-tag-group feeling-tag-group-${category.id}" ${open?'open':''}><summary><span><b>${category.emoji}</b><strong>${esc(t(category.label))}</strong></span><small>${tags.length}</small><i aria-hidden="true">›</i></summary><div class="feeling-tag-group-body">${tags.map(tag=>`<button type="button" class="feeling-tag ${chosen.has(tag.id)?'active':''}" data-observation-tag="${tag.id}"><span>${tag.emoji}</span>${esc(t(tag.label))}</button>`).join('')}</div></details>`}).join('')}
function recentObservationMeals(date,time,selected=[]){const end=new Date(`${date}T${time||'23:59'}:00`).getTime(),start=end-72*3600000,chosen=new Set(selected);return allMeals().filter(m=>{const at=mealDateTime(m).getTime();return at>=start&&at<=end}).sort((a,b)=>mealDateTime(b)-mealDateTime(a)).slice(0,12).map(m=>`<label class="observation-meal-option"><input type="checkbox" value="${m.id}" ${chosen.has(m.id)?'checked':''}><span><strong>${mealIcon(m.type,m.description)} ${esc(m.type)} · ${esc(m.time)}</strong><small>${esc(formatDate(m.date))} — ${esc(m.description)}</small></span></label>`).join('')||'<p class="muted small">Aucun repas enregistré dans les 72 heures précédentes.</p>'}
function makeObservationIntensity(value=3){const wrap=$("#globalObservationIntensity");wrap.dataset.value=String(value);wrap.innerHTML=[1,2,3,4,5].map(n=>`<button type="button" class="${n===Number(value)?'active':''}" data-observation-intensity="${n}"><span>${['','Très légère','Légère','Modérée','Forte','Très forte'][n]}</span><small>${n}</small></button>`).join('');$$('[data-observation-intensity]').forEach(b=>b.onclick=()=>{wrap.dataset.value=b.dataset.observationIntensity;$$('[data-observation-intensity]').forEach(x=>x.classList.toggle('active',x===b))})}
function showGlobalObservationPhoto(){const wrap=$("#globalObservationPhotoPreviewWrap");wrap.hidden=!globalObservationPhotoData;if(globalObservationPhotoData)$("#globalObservationPhotoPreview").src=globalObservationPhotoData}
function refreshObservationMeals(selected=[]){$("#globalObservationMeals").innerHTML=recentObservationMeals($("#globalObservationDate").value||selectedDate,$("#globalObservationTime").value||'23:59',selected)}
function openGlobalObservation(id=null){const found=id?findGlobalObservation(id):null,o=found?.observation||normalObservation({date:selectedDate,time:selectedDate===todayKey()?new Date().toTimeString().slice(0,5):'12:00'},selectedDate),readOnly=!!(db.settings?.demoMode&&db.settings?.demoReadOnly&&found);setDemoDetailReadOnly("#globalObservationForm",false);$("#globalObservationId").value=o.id;$("#globalObservationDate").value=o.date;$("#globalObservationDate").max=todayKey();$("#globalObservationTime").value=o.time;$("#globalObservationDuration").value=o.duration;$("#globalObservationNotes").value=o.notes||'';$("#globalObservationTags").innerHTML=observationTagPickerHtml(o.tags);$$('[data-observation-tag]').forEach(b=>b.onclick=()=>b.classList.toggle('active'));$$('#globalObservationForm .observation-contexts input').forEach(input=>input.checked=(o.contexts||[]).includes(input.value));makeObservationIntensity(o.intensity);refreshObservationMeals(o.mealIds);globalObservationPhotoData=o.photoLocal||null;$("#globalObservationPhoto").value='';showGlobalObservationPhoto();const remove=$("#deleteGlobalObservation");remove.hidden=!found;remove.onclick=found?()=>{if(confirm('Supprimer cette observation globale?')){found.day.observations=found.day.observations.filter(x=>x.id!==o.id);setDayChanged(found.date);$("#globalObservationDialog").close();render()}}:null;setDemoDetailReadOnly("#globalObservationForm",readOnly);$("#globalObservationDialog").showModal()}
...
$("#mealForm").onsubmit=e=>{e.preventDefault();const d=ensureDay(db,selectedDate),id=$("#mealId").value,old=d.meals.find(x=>x.id===id);const meal=normalMeal({...old,id:id||uid(),date:selectedDate,type:$("#mealType").value,time:$("#mealTime").value,description:$("#mealDescription").value.trim(),nutrition:db.settings.macroTracking?(nutritionFromInputs()||estimateNutritionFromText($("#mealDescription").value.trim())):(old?.nutrition||null),fatigueBefore:Number($("#fatigueBeforePicker").dataset.value),fatigueAfter:old?.fatigueAfter||0,notes:$("#mealNotes").value.trim(),photoLocal:photoData&&photoData.startsWith('data:')?photoData:(old?.photoLocal||null),photoUrl:photoRemoved?null:(old?.photoUrl||null),photoPath:photoRemoved?null:(old?.photoPath||null),updatedAt:new Date().toISOString()},selectedDate);const recommendation=["Déjeuner","Dîner","Souper"].includes(meal.type)?chooseMealRecommendation(selectedDate,meal):null;meal.recommendation=recommendation||null;if(old)Object.assign(old,meal);else d.meals.push(meal);const savedMeal=old||meal;setMealChanged(savedMeal);try{window.Brain?.learnMeal?.(savedMeal)}catch(memoryError){console.warn("Mémoire alimentaire",memoryError)}$("#mealDialog").close();render();if(recommendation)setTimeout(()=>showMealRecommendation(recommendation,selectedDate),280)};
...
$('#globalObservationDate').onchange=()=>refreshObservationMeals();$('#globalObservationTime').onchange=()=>refreshObservationMeals();
$('#globalObservationPhoto').onchange=async e=>{globalObservationPhotoData=await fileToDataUrl(e.target.files[0]);showGlobalObservationPhoto()};$('#removeGlobalObservationPhoto').onclick=()=>{globalObservationPhotoData=null;$('#globalObservationPhoto').value='';showGlobalObservationPhoto()};
$('#globalObservationForm').onsubmit=e=>{e.preventDefault();const id=$('#globalObservationId').value||uid(),date=$('#globalObservationDate').value,time=$('#globalObservationTime').value;if(!date||!time)return alert('Indique la date et le moment où tu as remarqué l’observation.');if(date>todayKey())return alert('La date de l’observation ne peut pas être dans le futur.');const tags=$$('[data-observation-tag].active').map(x=>x.dataset.observationTag);if(!tags.length&&!$('#globalObservationNotes').value.trim())return alert('Sélectionne au moins un élément observé ou ajoute une note.');const previous=findGlobalObservation(id),target=ensureDay(db,date),observation=normalObservation({...previous?.observation,id,date,time,intensity:Number($('#globalObservationIntensity').dataset.value)||3,duration:$('#globalObservationDuration').value,tags,contexts:$$('#globalObservationForm .observation-contexts input:checked').map(x=>x.value),mealIds:$$('#globalObservationMeals input:checked').map(x=>x.value),notes:$('#globalObservationNotes').value.trim(),photoLocal:globalObservationPhotoData,updatedAt:new Date().toISOString()},date);if(previous&&previous.date!==date){previous.day.observations=previous.day.observations.filter(x=>x.id!==id);setDayChanged(previous.date)}const index=target.observations.findIndex(x=>x.id===id);if(index>=0)target.observations[index]=observation;else target.observations.push(observation);setDayChanged(date);selectedDate=date;$('#globalObservationDialog').close();render()};
$('#sleepForm').onsubmit=e=>{e.preventDefault();const d=ensureDay(db,selectedDate),hours=parseAppNumber($('#sleepHours').value);if(hours!==null&&(hours<0||hours>24))return alert('Entre une durée de sommeil entre 0 et 24 heures.');d.sleepHours=hours;d.sleepTags=$$('[data-sleep-tag]:checked').map(input=>input.value);d.sleepComment=$('#sleepComment').value.trim();setDayChanged(selectedDate);$('#sleepDialog').close();render()};
$$('[data-sleep-tag]').forEach(input=>input.onchange=()=>{if(input.value==='none'&&input.checked)$$('[data-sleep-tag]').forEach(other=>{if(other!==input)other.checked=false});else if(input.checked){const none=$('[data-sleep-tag="none"]');if(none)none.checked=false}});
$('#activityForm').onsubmit=e=>{e.preventDefault();const d=ensureDay(db,selectedDate),type=$('#activityType').value,min=parseAppNumber($('#activityMinutes').value)||0,intensity=$('#activityIntensity').value||'moderate',actualRaw=$('#activityActualCalories').value.trim(),actualCalories=actualRaw===''?null:parseAppNumber(actualRaw);if(!type)return alert('Choisis un type d’activité.');if(min<1||min>1440)return alert('Indique une durée valide en minutes.');if(actualCalories!=null&&(actualCalories<0||actualCalories>10000))return alert('Indique une valeur de calories valide.');d.activities.push(normalizeActivity({id:uid(),type,minutes:min,intensity,estimatedCalories:estimateActivityCalories(type,min,intensity),actualCalories,at:new Date().toISOString()}));setDayChanged(selectedDate);$('#activityDialog').close();render()};
$$('[data-activity]').forEach(b=>b.onclick=()=>{$('#activityType').value=b.dataset.activity;$$('[data-activity]').forEach(x=>x.classList.toggle('active',x===b));updateActivityEstimate()});$$('[data-intensity]').forEach(b=>b.onclick=()=>setActivityIntensity(b.dataset.intensity));$('#activityMinutes').addEventListener('input',updateActivityEstimate);
...
$("#welcomeForm").onsubmit=e=>{e.preventDefault();$("#welcomeDialog").close()};$("#experienceStartDemo").onclick=()=>hasDemoAccess?startDemoMode("marie"):alert("Connecte-toi avec un compte autorisé pour ouvrir les profils de démonstration.");$("#experienceStartEmpty").onclick=startEmptyExperience;$("#nextDemoTour").onclick=()=>{demoTourIndex++;showDemoTourStep()};$("#previousDemoTour").onclick=()=>{if(demoTourIndex>0){demoTourIndex--;showDemoTourStep()}};$("#skipDemoTour").onclick=leaveDemoTourEarly;$("#closeDemoTour").onclick=leaveDemoTourEarly;$("#finishStartJournal").onclick=()=>{$("#demoFinalDialog")?.close();leaveDemoMode()};$("#finishExploreDemo").onclick=continueExploringDemo;$("#finishReplayTour").onclick=()=>{$("#demoFinalDialog")?.close();startDemoTour()};
$$('.close-dialog').forEach(b=>b.onclick=()=>b.closest('dialog').close());
document.addEventListener('click',event=>{const add=event.target.closest('#addGlobalObservation'),edit=event.target.closest('[data-global-observation]');if(add)openGlobalObservation();else if(edit)openGlobalObservation(edit.dataset.globalObservation)});
...
function splashLocalePack(){
 ...
 const packs={
  "fr-CA":{
   ...
   labels:{fact:"Saviez-vous que…",tip:"Astuce Énergie",appHint:"À découvrir"},
   facts:["Les fibres et les protéines contribuent souvent à une satiété plus durable.","La régularité du sommeil compte souvent autant que sa durée.","Une courte marche après un repas peut aider certaines personnes à se sentir plus alertes.","Les besoins en énergie varient selon le sommeil, l’activité et plusieurs autres facteurs.","Un même repas peut être ressenti différemment selon le sommeil, le stress ou l’activité de la journée.","Les tendances deviennent plus utiles lorsqu’on compare plusieurs journées plutôt qu’un seul repas.","Noter le moment d’un ressenti aide à observer les réactions qui apparaissent plus tard."],
   ...
   appHints:["Le Cerveau compare plusieurs journées pour repérer des habitudes; il ne tire jamais de conclusion à partir d’une seule entrée.","Le Profil regroupe tes préférences, tes favoris, la langue, les notifications et les options de confidentialité.","Plus ton journal est complet — repas, sommeil, hydratation, activité et ressentis — plus les tendances deviennent utiles.","Une tendance décrit une évolution répétée dans le temps; ce n’est ni un diagnostic ni une certitude médicale.","Le Tableau intelligent résume les sept derniers jours pour t’aider à voir rapidement ce qui change.","Les Observations permettent de noter un symptôme ou un ressenti, même lorsqu’il n’est pas lié à un repas précis.","Les Favoris servent à réutiliser rapidement les repas que tu manges souvent, sans tout retaper.","Les estimations nutritionnelles sont approximatives et servent surtout à comparer les habitudes dans le temps.","Le Cerveau devient plus pertinent à mesure que ton historique s’allonge et que tes entrées restent régulières.","Tu peux revenir modifier une journée passée afin de garder ton historique aussi fidèle que possible."]
  },
  "fr-FR":{
   ...
   labels:{fact:"Le saviez-vous ?",tip:"Astuce Énergie",appHint:"À découvrir"},
   facts:["Les fibres et les protéines contribuent souvent à une satiété plus durable.","La régularité du sommeil compte souvent autant que sa durée.","Une courte marche après un repas peut aider certaines personnes à se sentir plus alerte.","Les besoins en énergie varient selon le sommeil, l’activité et plusieurs autres facteurs.","Un même repas peut être ressenti différemment selon le sommeil, le stress ou l’activité de la journée.","Les tendances deviennent plus utiles lorsque l’on compare plusieurs journées plutôt qu’un seul repas.","Noter le moment d’un ressenti aide à observer les réactions qui apparaissent plus tard."],
   ...
   appHints:["Le Cerveau compare plusieurs journées pour repérer des habitudes ; il ne tire jamais de conclusion à partir d’une seule entrée.","Le Profil regroupe vos préférences, vos favoris, la langue, les notifications et les options de confidentialité.","Plus votre journal est complet — repas, sommeil, hydratation, activité et ressentis — plus les tendances deviennent utiles.","Une tendance décrit une évolution répétée dans le temps ; ce n’est ni un diagnostic ni une certitude médicale.","Le Tableau intelligent résume les sept derniers jours pour vous aider à voir rapidement ce qui change.","Les Observations permettent de noter un symptôme ou un ressenti, même lorsqu’il n’est pas lié à un repas précis.","Les Favoris servent à réutiliser rapidement les repas que vous mangez souvent, sans tout ressaisir.","Les estimations nutritionnelles sont approximatives et servent surtout à comparer les habitudes dans le temps.","Le Cerveau devient plus pertinent à mesure que votre historique s’allonge et que vos entrées restent régulières.","Vous pouvez revenir modifier une journée passée afin de garder votre historique aussi fidèle que possible."]
  },
  en:{
   ...
   labels:{fact:"Did you know?",tip:"Energy tip",appHint:"Discover"},
   facts:["Fiber and protein often contribute to longer-lasting fullness.","A consistent sleep schedule can matter as much as sleep duration.","A short walk after a meal may help some people feel more alert.","Energy needs vary with sleep, activity, and many other factors.","The same meal may feel different depending on sleep, stress, or activity that day.","Patterns become more useful when several days are compared instead of a single meal.","Recording when a symptom begins helps capture reactions that appear later."],
   ...
   appHints:["The Brain compares several days to identify patterns; it never draws a conclusion from a single entry.","Profile brings together your preferences, favorites, language, notifications, and privacy options.","The more complete your journal is — meals, sleep, hydration, activity, and symptoms — the more useful your trends become.","A trend describes a repeated change over time; it is not a diagnosis or a medical certainty.","The Smart Dashboard summarizes your last seven days so you can quickly see what is changing.","Observations let you log a symptom or feeling even when it is not linked to a specific meal.","Favorites help you reuse meals you eat often without typing everything again.","Nutrition estimates are approximate and are mainly intended to compare habits over time.","The Brain becomes more relevant as your history grows and your entries remain consistent.","You can edit a past day to keep your history as accurate as possible."]
  }
 };
...
async function loadDemoAccess(){
  ...
  try{
    const {data,error}=await client.from("profiles").select("has_demo_access").eq("id",session.user.id).maybeSingle();
    if(error){console.info("Accès démo non configuré:",error.message);return false}
    hasDemoAccess=data?.has_demo_access===true;
  }catch(error){console.info("Accès démo indisponible:",error?.message||error)}
  return hasDemoAccess
}
async function initAuth(){if(!client){render();setTimeout(showExperienceLaunchIfNeeded,120);return}const {data}=await client.auth.getSession();session=data.session;client.auth.onAuthStateChange((event,newSession)=>{session=newSession;if(newSession)loadDemoAccess().then(()=>render());else hasDemoAccess=false;updateSyncBadge();if(event==="PASSWORD_RECOVERY")setTimeout(()=>$("#passwordDialog").showModal(),0);if(event==="SIGNED_OUT")render()});if(session){await loadDemoAccess();if(!db.settings.demoMode){await pullCloud(false);await syncNow()}}render();setTimeout(showExperienceLaunchIfNeeded,120)}
