...
$("#mealForm").onsubmit=e=>{e.preventDefault();const d=ensureDay(db,selectedDate),id=$("#mealId").value,old=d.meals.find(x=>x.id===id);const meal=normalMeal({...old,id:id||uid(),date:selectedDate,type:$("#mealType").value,time:$("#mealTime").value,description:$("#mealDescription").value.trim(),nutrition:db.settings.macroTracking?(nutritionFromInputs()||estimateNutritionFromText($("#mealDescription").value.trim())):(old?.nutrition||null),fatigueBefore:Number($("#fatigueBeforePicker").dataset.value),fatigueAfter:old?.fatigueAfter||0,notes:$("#mealNotes").value.trim(),photoLocal:photoData&&photoData.startsWith('data:')?photoData:(old?.photoLocal||null),photoUrl:photoRemoved?null:(old?.photoUrl||null),photoPath:photoRemoved?null:(old?.photoPath||null),updatedAt:new Date().toISOString()},selectedDate);const isMainMeal=["Déjeuner","Dîner","Souper"].includes(meal.type);const recommendation=isMainMeal?chooseMealRecommendation(selectedDate,meal):null;if(recommendation)meal.recommendation=recommendation;else if(!isMainMeal)meal.recommendation=null;if(old)Object.assign(old,meal);else d.meals.push(meal);const savedMeal=old||meal;setMealChanged(savedMeal);$("#mealDialog").close();render();if(recommendation)setTimeout(()=>showMealRecommendation(recommendation,selectedDate),280)};
