...
function ensureDay(store,key=todayKey()){if(!store.days[key])store.days[key]={date:key,sleepHours:null,sleepTags:[],sleepComment:"",water:0,activities:[],meals:[],updatedAt:new Date().toISOString()};const d=store.days[key];d.activities=Array.isArray(d.activities)?d.activities:[];d.meals=Array.isArray(d.meals)?d.meals:[];d.sleepTags=Array.isArray(d.sleepTags)?d.sleepTags:[];d.sleepComment=typeof d.sleepComment==="string"?d.sleepComment:"";d.water=Number(d.water)||0;return d}
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./sw.js?v=3.0.5');await reg.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()});if(reg.waiting)reg.waiting.postMessage?.({type:'SKIP_WAITING'})}catch(e){console.warn(e)}});
dismissSplash();
initAuth();
