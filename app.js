...
function renderDemoChrome(){
  ...
  const launchButton=$("#launchDemoHeader");
  if(launchButton){
    launchButton.hidden=!!db.settings.demoMode;
    launchButton.setAttribute("aria-hidden",db.settings.demoMode?"true":"false")
  }
  const appRoot=$("#app");
...
function startDemoMode(){
  try{
    ...
    console.warn("La copie avant démo n’a pas pu être enregistrée:",error)
  }
  ...
  try{
    ...
    try{
      ...
      console.warn("La démo fonctionne sans être enregistrée dans le stockage local:",error)
    }

    requestAnimationFrame(()=>{
      requestAnimationFrame(()=>startDemoTour())
    })
  }catch(error){
    console.error("Impossible de lancer la démo:",error);
    alert("La démo n’a pas pu démarrer. Recharge la page et réessaie. Détail: "+(error?.message||error))
  }
}
document.addEventListener("click",event=>{
  const trigger=event.target.closest?.("#startDemo,#launchDemoHeader,#launchDemoProfile");
  if(!trigger)return;
...
function leaveDemoMode(){
  ...
  try{
    ...
    console.warn("Impossible de restaurer la copie avant démo:",error)
  }
  db=restored||freshDB();
...
const demoTourSteps=[
  {view:"today",target:".journal-progress, .journal-date-nav",title:"Bienvenue dans le journal de Phil",text:"Pendant une minute, je vais te montrer comment de petites notes quotidiennes deviennent des observations utiles."},
  ...
  {view:"history",target:".timeline, .history-toolbar, main",title:"L’historique montre les répétitions",text:"Après plusieurs semaines, les journées s’accumulent et les habitudes deviennent faciles à reconnaître."},
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./sw.js?v=2.5.4');await reg.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()});if(reg.waiting)reg.waiting.postMessage?.({type:'SKIP_WAITING'})}catch(e){console.warn(e)}});
dismissSplash();
initAuth();
