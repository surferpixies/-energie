...
function ensureDay(store,key=todayKey()){if(!store.days[key])store.days[key]={date:key,sleepHours:null,sleepTags:[],sleepComment:"",water:0,activities:[],meals:[],supplementsTaken:[],updatedAt:new Date().toISOString()};const d=store.days[key];d.activities=(Array.isArray(d.activities)?d.activities:[]).map(normalizeActivity);d.meals=Array.isArray(d.meals)?d.meals:[];d.sleepTags=Array.isArray(d.sleepTags)?d.sleepTags:[];d.sleepComment=typeof d.sleepComment==="string"?d.sleepComment:"";d.water=Number(d.water)||0;const defaults=normalizeSupplements(store.settings?.supplements||[]);d.supplementsTaken=normalizeSupplements(d.supplementsTaken||d.supplements||[]);if(!d.supplementsTaken.length&&defaults.length)d.supplementsTaken=[...defaults];return d}
...
function installDemoReadOnlyGuard(){
  ...
  document.addEventListener("submit",event=>{
    if(!db.settings?.demoMode||!db.settings?.demoReadOnly)return;
    if(event.target.closest("#app")||event.target.matches("#mealForm,#sleepForm,#activityForm,#feelingForm")){
...
function nutritionEstimateMarkup(n){
 n=normalNutrition(n);
 if(!n)return"";
 const calorieText=n.calories!=null?`≈ ${Math.round(n.calories)} kcal`:`≈ ${t("Valeurs approximatives")}`;
 const metrics=[
  ["🥩",t("Protéines"),n.protein,"g"],
  ["🍞",t("Glucides"),n.carbs,"g"],
  ["🧈",t("Lipides"),n.fat,"g"],
  ["🌾",t("Fibres"),n.fiber,"g"],
  ["🍬",t("Sucres"),n.sugars,"g"],
  ["🧂",t("Sodium"),n.sodium,"mg"]
 ].filter(([, ,value])=>value!=null);
 const rows=metrics.map(([icon,label,value,unit])=>{
  const shown=unit==="mg"?Math.round(value):Number(value).toFixed(value%1?1:0);
  return `<div class="meal-nutrition-row"><span>${icon}</span><span>${esc(label)}</span><strong>${shown} ${unit}</strong></div>`;
 }).join("");
 return `<details class="meal-nutrition-compact"><summary><span class="meal-nutrition-summary"><b>🔥</b><span><small>${esc(t("Estimation nutritionnelle"))}</small><strong>${esc(calorieText)}</strong></span></span><span class="weekly-trends-chevron">›</span></summary><div class="meal-nutrition-details">${rows||`<p class="meal-nutrition-empty">${esc(t("Valeurs approximatives"))}</p>`}<p class="meal-nutrition-note">${esc(t("Valeurs approximatives, calculées à partir des aliments reconnus."))}</p></div></details>`;
}
function mealCard(m,opts={}){const feeling=m.feeling;const feelingEligible=isFeelingEligible(m);const feelingPreview=feelingEligible?`<div class="meal-feeling-preview ${feeling?'is-set':'is-empty'}">${feeling?`<span>${feelingEmoji(feeling.rating)} ${feeling.rating}/5</span>${feeling.tags?.length?`<small>${feeling.tags.length} étiquette${feeling.tags.length>1?'s':''}</small>`:''}`:`<span>Ressenti</span><small>À noter</small>`}</div>`:"";return `<article class="card meal-card" data-meal="${m.id}" data-date="${m.date}"><div class="meal-thumb">${m.photoUrl||m.photoLocal?`<img src="${esc(m.photoUrl||m.photoLocal)}" alt="">`:mealIcon(m.type,m.description)}</div><div class="meal-card-body"><h3>${esc(m.description)}</h3><div class="meal-meta">${esc(m.time)} · ${esc(t(m.type))}${opts.showDate?` · ${esc(formatDate(m.date))}`:""}</div>${db.settings.macroTracking&&m.nutrition?nutritionEstimateMarkup(m.nutrition):""}${feelingPreview}<div class="meal-footer"><span class="chip">⚡ ${m.fatigueBefore||'—'}/5</span>${feelingEligible?`<button class="meal-feeling-inline ${feeling?'is-set':'is-empty'}" data-feeling="${m.id}" title="${feeling?'Modifier le ressenti':'Ajouter un ressenti'}">${feeling?`${feelingEmoji(feeling.rating)} ${feeling.rating}/5`:'Ressenti'}</button>`:""}</div></div><div class="meal-actions">${feelingEligible?`<button class="feeling-meal" data-feeling="${m.id}" title="${feeling?'Modifier le ressenti':'Ajouter un ressenti'}">${feeling?'😊':'＋😊'}</button>`:""}<button class="favorite-meal" data-favorite="${m.id}" title="Ajouter aux favoris">☆</button><button class="delete-meal" data-delete="${m.id}" title="Supprimer">×</button></div></article>`}
...
function dailyMacroSummaryHtml(meals){
  ...
  const extra=[{key:"fiber",icon:"🌾",label:"Fibres",unit:"g"},{key:"sugars",icon:"🍬",label:"Sucres",unit:"g"},{key:"sodium",icon:"🧂",label:"Sodium",unit:"mg"}].filter(x=>known.every(m=>m[x.key]!=null)).map(x=>{const value=Math.round(sum(x.key)*10)/10;return `<span><b>${x.icon} ${value.toLocaleString('fr-CA')} ${x.unit}</b><small>${x.label}</small></span>`}).join("");return `<section class="daily-macros-card" aria-label="Estimation nutritionnelle de la journée"><div class="daily-macros-head"><span>Estimation nutritionnelle</span><strong>≈ ${calories.toLocaleString('fr-CA')} kcal</strong></div><div class="daily-macros-grid"><span><b>🥩 ${protein.toLocaleString('fr-CA')} g</b><small>Protéines</small></span><span><b>🍞 ${carbs.toLocaleString('fr-CA')} g</b><small>Glucides</small></span><span><b>🥑 ${fat.toLocaleString('fr-CA')} g</b><small>Lipides</small></span>${extra}</div><p>Valeurs approximatives, calculées à partir des aliments reconnus. ${coverage}</p></section>`;
}
function supplementsTodayHtml(day){const defaults=normalizeSupplements(db.settings?.supplements||[]);if(!defaults.length)return "";const taken=normalizeSupplements(day?.supplementsTaken||[]);const items=defaults.map(name=>`<label class="supplement-toggle"><input type="checkbox" data-supplement-name="${esc(name)}" ${taken.includes(name)?'checked':''}><span>${esc(name)}</span></label>`).join("");return `<section class="card supplement-card"><div class="row"><div class="hydration-heading"><span>💊</span><h3>Suppléments</h3></div><strong>${taken.length}/${defaults.length}</strong></div><p class="muted small">Coche ce que tu as pris aujourd’hui. Les éléments restants restent décochés.</p><div class="supplement-list">${items}</div></section>`}
