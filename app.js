...
function ensureDay(store,key=todayKey()){if(!store.days[key])store.days[key]={date:key,sleepHours:null,sleepTags:[],sleepComment:"",water:0,activities:[],meals:[],updatedAt:new Date().toISOString()};const d=store.days[key];d.activities=(Array.isArray(d.activities)?d.activities:[]).map(normalizeActivity);d.meals=Array.isArray(d.meals)?d.meals:[];d.sleepTags=Array.isArray(d.sleepTags)?d.sleepTags:[];d.sleepComment=typeof d.sleepComment==="string"?d.sleepComment:"";d.water=Number(d.water)||0;return d}
...
function migrate(raw){const out=freshDB();if(!raw||typeof raw!=="object")return out;out.settings={...out.settings,...(raw.settings||{})};if(Number(raw.version||0)<19&&raw.settings?.macroTracking===false)out.settings.macroTracking=true;out.favorites=(raw.favorites||raw.favoriteMeals||[]).map(normalFavorite);if(raw.days&&typeof raw.days==="object"){Object.entries(raw.days).forEach(([k,d])=>{const day=ensureDay(out,k);day.sleepHours=d.sleepHours??d.sleep??d.sommeil??null;day.sleepTags=Array.isArray(d.sleepTags)?d.sleepTags:(Array.isArray(d.sleep_tags)?d.sleep_tags:[]);day.sleepComment=d.sleepComment??d.sleep_comment??"";day.water=Number(d.water??d.waterGlasses??d.eau??0)||0;day.activities=(Array.isArray(d.activities)?d.activities:[]).map(normalizeActivity);day.meals=(d.meals||d.repas||[]).map(m=>normalMeal(m,k));day.updatedAt=d.updatedAt||new Date().toISOString()});return out}const arr=[raw.meals,raw.repas,raw.entries,raw.history,raw.logs,Array.isArray(raw)?raw:null].find(Array.isArray);if(arr)arr.forEach(x=>{const m=normalMeal(x,x.date||x.day||todayKey());ensureDay(out,m.date).meals.push(m)});return out}
...
function historyNegativeFeelingStats(meals,days=30){const cutoff=new Date();cutoff.setDate(cutoff.getDate()-days+1);cutoff.setHours(0,0,0,0);const symptomMeta=Object.fromEntries(FEELING_TAGS.filter(tag=>tag.group==='symptom').map(tag=>[tag.id,tag]));const byDate={};meals.forEach(meal=>{if(!meal?.date)return;(byDate[meal.date]??=[]).push(meal)});const series=[];for(let i=0;i<days;i++){const day=new Date(cutoff);day.setDate(cutoff.getDate()+i);const key=`${day.getFullYear()}-${String(day.getMonth()+1).padStart(2,'0')}-${String(day.getDate()).padStart(2,'0')}`;const dayMeals=(byDate[key]||[]).slice().sort((a,b)=>a.time.localeCompare(b.time));const count=dayMeals.reduce((sum,meal)=>sum+(meal.feeling?.tags||[]).filter(tag=>symptomMeta[tag]).length,0);series.push({date:key,label:day.toLocaleDateString('fr-CA',{day:'numeric'}),count})}const symptoms={};meals.filter(meal=>meal.feeling&&Array.isArray(meal.feeling.tags)&&meal.feeling.tags.length).forEach(meal=>{const occurrenceDate=new Date(`${meal.date}T12:00:00`);if(occurrenceDate<cutoff)return;const dayMeals=(byDate[meal.date]||[]).slice().sort((a,b)=>a.time.localeCompare(b.time));const mealIndex=dayMeals.findIndex(x=>x.id===meal.id);const priorMeals=mealIndex>=0?dayMeals.slice(Math.max(0,mealIndex-3),mealIndex):[];meal.feeling.tags.filter(tag=>symptomMeta[tag]).forEach(tag=>{const entry=symptoms[tag]||{id:tag,label:symptomMeta[tag].label,emoji:symptomMeta[tag].emoji,count:0,occurrences:[]};entry.count+=1;entry.occurrences.push({date:meal.date,mealDescription:meal.description||'Repas',mealTime:meal.time||'12:00',mealType:t(meal.type),rating:meal.feeling.rating,notes:meal.feeling.notes||'',priorMeals:priorMeals.map(({description,time,type})=>({description,time,type}))});symptoms[tag]=entry})});return{series,symptoms:Object.values(symptoms).sort((a,b)=>b.count-a.count)}}
function historyNegativeFeelingHtml(stats){if(!stats.symptoms.length)return `<section class="card history-feelings-card"><div class="history-feelings-empty"><p class="eyebrow">Ressentis négatifs</p><h3>Aucun signalement sur les 30 derniers jours</h3><p class="muted small">Les symptômes enregistrés ici apparaîtront automatiquement ici.</p></div></section>`;const maxCount=Math.max(1,...stats.series.map(day=>day.count));return `<section class="card history-feelings-card"><div class="history-feelings-head"><div><p class="eyebrow">Ressentis négatifs</p><h3>Les 30 derniers jours</h3></div><span class="muted small">${stats.symptoms.reduce((sum,item)=>sum+item.count,0)} signalements</span></div><div class="history-feelings-body"><div class="history-feelings-chart"><div class="history-feelings-bars">${stats.series.map(day=>`<div class="history-feelings-bar-column"><div class="history-feelings-bar" style="height:${Math.max(6,Math.round((day.count/maxCount)*100))}%"></div><small>${esc(day.label)}</small></div>`).join('')}</div></div><div class="history-feelings-list">${stats.symptoms.slice(0,6).map(item=>`<div class="history-feeling-pill"><span>${item.emoji}</span><div><strong>${esc(item.label)}</strong><small>${item.count} fois</small></div></div>`).join('')}</div></div><div class="history-feelings-details">${stats.symptoms.map(item=>`<details class="history-feeling-detail"><summary><span class="history-feeling-detail-title">${item.emoji} ${esc(item.label)}</span><b>${item.count}</b></summary><div class="history-feeling-detail-body">${item.occurrences.map(occ=>`<article class="history-feeling-occurrence"><div class="history-feeling-occurrence-head"><strong>${esc(formatDate(occ.date))}</strong><span>${esc(occ.mealTime)} · ${esc(occ.mealType)}</span></div><p>${esc(occ.mealDescription)} · ${occ.rating}/5${occ.notes?` · ${esc(occ.notes)}`:''}</p>${occ.priorMeals.length?`<div class="history-feeling-context"><span>3 repas précédents</span>${occ.priorMeals.map(meal=>`<div class="history-feeling-context-item"><strong>${esc(meal.description)}</strong><small>${esc(meal.time)} · ${esc(t(meal.type))}</small></div>`).join('')}</div>`:`<p class="muted small">Pas assez de repas précédents sur cette journée.</p>`}</article>`).join('')}</div></details>`).join('')}</div></section>`}
function dashboardNegativeFeelingHtml(stats){if(!stats.symptoms.length)return `<section class="card wide dashboard-feelings-card"><div class="history-feelings-head"><div><p class="eyebrow">Ressentis négatifs</p><h3>Aucun signalement</h3></div></div><div class="history-feelings-empty"><p class="muted small">Aucun symptôme n’a été enregistré sur les 30 derniers jours.</p></div></section>`;const maxCount=Math.max(1,...stats.series.map(day=>day.count));return `<section class="card wide dashboard-feelings-card"><div class="history-feelings-head"><div><p class="eyebrow">Ressentis négatifs</p><h3>Les 30 derniers jours</h3></div><span class="muted small">${stats.symptoms.reduce((sum,item)=>sum+item.count,0)} signalements</span></div><div class="history-feelings-body dashboard-feelings-body"><div class="history-feelings-chart"><div class="history-feelings-bars">${stats.series.map(day=>`<div class="history-feelings-bar-column"><div class="history-feelings-bar" style="height:${Math.max(6,Math.round((day.count/maxCount)*100))}%"></div><small>${esc(day.label)}</small></div>`).join('')}</div></div><div class="history-feelings-list dashboard-feelings-list">${stats.symptoms.slice(0,4).map(item=>`<div class="history-feeling-pill dashboard-feeling-pill"><span>${item.emoji}</span><div><strong>${esc(item.label)}</strong><small>${item.count} fois</small></div></div>`).join('')}</div></div><div class="history-feelings-details">${stats.symptoms.map(item=>`<details class="history-feeling-detail"><summary><span class="history-feeling-detail-title">${item.emoji} ${esc(item.label)}</span><b>${item.count}</b></summary><div class="history-feeling-detail-body">${item.occurrences.slice(0,2).map(occ=>`<article class="history-feeling-occurrence"><div class="history-feeling-occurrence-head"><strong>${esc(formatDate(occ.date))}</strong><span>${esc(occ.mealTime)} · ${esc(occ.mealType)}</span></div><p>${esc(occ.mealDescription)} · ${occ.rating}/5${occ.notes?` · ${esc(occ.notes)}`:''}</p>${occ.priorMeals.length?`<div class="history-feeling-context"><span>3 repas précédents</span>${occ.priorMeals.map(meal=>`<div class="history-feeling-context-item">${esc(meal.description)} · ${esc(meal.time)}</div>`).join('')}</div>`:''}</article>`).join('')}</div></details>`).join('')}</div></section>`}
