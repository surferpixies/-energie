  ...
  function calorieTargetGaugeHtml(date, meals, options = {}) {
    ...
    if (options.circular) {
      ...
      const metaDetail = options.daily
        ...
        : `${esc(status)}${esc(partial)} · déficit visé ${targetData.deficit.toLocaleString("fr-CA")} kcal`;
      return `<div class="calorie-target-ring-card ${options.daily ? "is-daily" : ""} ${state} ${future ? "is-future" : ""}"><div class="calorie-target-ring" style="--calorie-progress-pct:${pct}" role="progressbar" aria-label="${esc(lead)} ${consumed} calories sur une cible estimée de ${targetData.target}" aria-valuemin="0" aria-valuemax="${targetData.target}" aria-valuenow="${Math.min(consumed, targetData.target)}"><svg class="calorie-target-ring-svg" viewBox="0 0 44 44" aria-hidden="true"><circle class="calorie-target-ring-track" cx="22" cy="22" r="18" pathLength="100"></circle><circle class="calorie-target-ring-progress" cx="22" cy="22" r="18" pathLength="100" style="--ring-pct:${pct}"></circle></svg><div class="calorie-target-ring-center"><strong>${consumed.toLocaleString("fr-CA")}</strong><small>kcal</small></div></div><div class="calorie-target-ring-meta"><strong>${metaTitle}</strong><small>${metaDetail}</small></div></div>`;
    }
