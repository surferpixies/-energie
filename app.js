...
function learnedHabitInsight(meals){
 ...
 const candidates=[];
 if(dayCount>=5&&breakfastDays/dayCount>=.75)candidates.push({score:breakfastDays/dayCount,icon:"🥣",text:"Tu prends un déjeuner presque tous les jours.",n:dayCount,basis:`Observation basée sur ${dayCount} jours des 28 derniers jours.`});
 if(snacks.length>=3&&fruitSnacks/snacks.length>=.6)candidates.push({score:fruitSnacks/snacks.length+.05,icon:"🍎",text:"Tu choisis souvent un fruit comme collation.",n:snacks.length,basis:`Observation basée sur ${snacks.length} collations des 28 derniers jours.`});
 if(dinners.length>=4&&vegetableDinners/dinners.length>=.6)candidates.push({score:vegetableDinners/dinners.length+.04,icon:"🥦",text:"Les légumes sont présents dans la majorité de tes soupers.",n:dinners.length,basis:`Observation basée sur ${dinners.length} soupers des 28 derniers jours.`});
 if(main.length>=6&&proteinMeals/main.length>=.65)candidates.push({score:proteinMeals/main.length+.03,icon:"🍗",text:"Une source de protéines apparaît dans la plupart de tes repas principaux.",n:main.length,basis:`Observation basée sur ${main.length} repas principaux des 28 derniers jours.`});
 if(daysWithWater.length>=5&&waterGoalDays/daysWithWater.length>=.7)candidates.push({score:waterGoalDays/daysWithWater.length+.02,icon:"💧",text:"Ton hydratation est très constante les jours où tu la documentes.",n:daysWithWater.length,basis:`Observation basée sur ${daysWithWater.length} jours d’hydratation des 28 derniers jours.`});
 const best=candidates.sort((a,b)=>b.score-a.score)[0];
 if(!best)return{icon:"📖",title:"Ce que j’apprends sur toi",text:"Ton journal devient assez complet pour faire ressortir progressivement des habitudes plus utiles.",confidence:insightConfidence(recent.length),basis:`Basé sur ${recent.length} entrées des 28 derniers jours.`,kind:"personal"};
...
function buildPersonalInsights(meals){const cards=[];if(!meals.length)return cards;const valid=meals.filter(m=>m.fatigueBefore>0);const byType={};valid.forEach(m=>(byType[m.type]??=[]).push(m.fatigueBefore));const least=Object.entries(byType).filter(([,v])=>v.length>=2).map(([k,v])=>[k,average(v),v.length]).sort((a,b)=>b[1]-a[1])[0];if(least){cards.push({icon:"🍽️",title:"Énergie observée avant certains repas",text:`Avant tes repas « ${least[0]} », ton énergie enregistrée est en moyenne de ${least[1].toFixed(1)}/5. Cela décrit ton historique sans expliquer la cause.`,confidence:insightConfidence(least[2]),basis:`Basé sur ${least[2]} repas enregistrés.`,kind:"personal"})}const learned=learnedHabitInsight(meals);if(learned)cards.push(learned);const weekdays={};valid.forEach(m=>{const day=new Date(`${m.date}T12:00:00`).toLocaleDateString('fr-CA',{weekday:'long'});(weekdays[day]??=[]).push(m.fatigueBefore)});const best=Object.entries(weekdays).filter(([,v])=>v.length>=2).map(([k,v])=>[k,average(v),v.length]).sort((a,b)=>b[1]-a[1])[0];if(best)cards.push({icon:"📅",title:"Journée associée à plus d’énergie",text:`Le ${best[0]}, ton énergie avant les repas est en moyenne de ${best[1].toFixed(1)}/5 dans les données disponibles.`,confidence:insightConfidence(best[2]),basis:`Basé sur ${best[2]} repas enregistrés ce jour de la semaine.`,kind:"personal"});return cards.slice(0,3)}
