...
function normalizeActivity(a={}){const type=a.type||"Autre",minutes=Math.max(0,Number(a.minutes)||0),intensity=["low","moderate","high"].includes(a.intensity)?a.intensity:"moderate";const estimatedCalories=Number.isFinite(Number(a.estimatedCalories))?Math.max(0,Math.round(Number(a.estimatedCalories))):estimateActivityCalories(type,minutes,intensity);const actualRaw=a.actualCalories??a.caloriesActual??null;const actualCalories=actualRaw===""||actualRaw==null?null:Math.max(0,Math.round(Number(actualRaw)||0));return{id:a.id||uid(),type,minutes,intensity,estimatedCalories,actualCalories,at:a.at||new Date().toISOString()}}
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./sw.js?v=3.5.0a');await reg.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()});if(reg.waiting)reg.waiting.postMessage?.({type:'SKIP_WAITING'})}catch(e){console.warn(e)}});
dismissSplash();
initAuth();
