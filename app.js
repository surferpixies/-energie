...
let barcodeReader=null,barcodeControls=null,barcodeBusy=false,barcodeLastCode="";
...
function resetBarcodeResult(){barcodeLastCode="";$("#barcodeResult").hidden=true;$("#barcodeProductName").value="";$("#barcodeBrand").textContent="";$("#barcodeProductImage").hidden=true;$("#barcodeProductImage").removeAttribute("src")}
...
function showBarcodeProduct(product,code,found=true){barcodeLastCode=code;const result=$("#barcodeResult"),name=$("#barcodeProductName"),brand=$("#barcodeBrand"),image=$("#barcodeProductImage"),badge=$("#barcodeResultBadge"),help=$("#barcodeResultHelp");result.hidden=false;name.value=product.name||"";brand.textContent=product.brand||"";badge.textContent=found?"Produit reconnu":"Produit inconnu";help.textContent=found?"Tu peux simplifier ou modifier le nom avant de l’ajouter.":"Écris simplement le nom que tu veux ajouter au repas.";if(product.image){image.src=product.image;image.hidden=false}else{image.hidden=true;image.removeAttribute("src")}setTimeout(()=>name.focus(),80)}
async function lookupBarcode(code){code=cleanBarcode(code);if(code.length<6){alert("Entre un numéro de code-barres valide.");return}barcodeBusy=true;setBarcodeStatus("Recherche du produit…");const cached=barcodeCache()[code];if(cached?.name){showBarcodeProduct(cached,code,true);setBarcodeStatus("Produit trouvé dans tes scans récents.");barcodeBusy=false;stopBarcodeCamera();return}try{const url=`https://world.openfoodfacts.org/api/v2/product/${encodeURIComponent(code)}.json?fields=code,product_name,product_name_fr,product_name_en,brands,image_front_small_url,image_front_url`;const response=await fetch(url,{headers:{Accept:"application/json"}});if(!response.ok)throw new Error(`HTTP ${response.status}`);const data=await response.json(),p=data?.product;if(data?.status===1&&p){const product={name:(p.product_name_fr||p.product_name||p.product_name_en||"").trim(),brand:(p.brands||"").split(",")[0].trim(),image:p.image_front_small_url||p.image_front_url||""};if(product.name){saveBarcodeCache(code,product);showBarcodeProduct(product,code,true);setBarcodeStatus("Produit reconnu.")}else{showBarcodeProduct({},code,false);setBarcodeStatus("Le produit existe, mais son nom est manquant.")}}else{showBarcodeProduct({},code,false);setBarcodeStatus("Produit non trouvé. Tu peux entrer son nom.")}stopBarcodeCamera()}catch(e){console.warn("Open Food Facts",e);showBarcodeProduct({},code,false);setBarcodeStatus("Recherche impossible pour le moment. Entre le nom manuellement.");stopBarcodeCamera()}finally{barcodeBusy=false}}
...
$("#addBarcodeProduct").onclick=()=>{const name=$("#barcodeProductName").value.trim();if(!name){$("#barcodeProductName").focus();return}appendScannedFood(name);if(barcodeLastCode)saveBarcodeCache(barcodeLastCode,{name,brand:$("#barcodeBrand").textContent.trim(),image:$("#barcodeProductImage").hidden?"":$("#barcodeProductImage").src});closeBarcodeScanner();$("#mealDescription").focus()};
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./sw.js?v=2.7.0');await reg.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()});if(reg.waiting)reg.waiting.postMessage?.({type:'SKIP_WAITING'})}catch(e){console.warn(e)}});
dismissSplash();
initAuth();
