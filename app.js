...
async function pullMemoryCloud(){if(!client||!session||!navigator.onLine||!window.Brain?.memory)return true;try{const {data,error}=await client.from(MEMORY_CLOUD_TABLE).select("*").order("last_seen_at",{ascending:false});if(error)throw error;const merged=mergeMemoryStates(brainMemoryState(),data||[]);window.Brain.replaceMemoryState?.(merged);if((merged.memories||[]).length)await syncMemoryCloud();return true}catch(error){console.warn("Chargement de la mémoire alimentaire",error);return false}}
