...
function snackIconForText(text){const clean=String(text||"").toLocaleLowerCase("fr-CA");const patterns=[{regex:/\b(yogourt|yaourt)\b/,icon:"🥣"},{regex:/\b(pomme|pomme rouge|pomme verte)\b/,icon:"🍎"},{regex:/\b(kiwi|kiwis)\b/,icon:"🥝"},{regex:/\b(banane)\b/,icon:"🍌"},{regex:/\b(orange|mandarine|clémentine|clementine)\b/,icon:"🍊"},{regex:/\b(poire)\b/,icon:"🍐"},{regex:/\b(fraise|fraises)\b/,icon:"🍓"},{regex:/\b(fromage|cheese|feta|mozzarella|brie|camembert)\b/,icon:"🧀"},{regex:/\b(noix|noix de cajou|pistache|amande|amandes|cacahu|cajou|noix de coco)\b/,icon:"🥜"},{regex:/\b(barre|barre tendre|protein bar|snack bar|chocolate bar|barre de chocolat)\b/,icon:"🍫"},{regex:/\b(cookie|cookies|biscuit|biscuits|cracker|crackers|galette|galettes)\b/,icon:"🍪"},{regex:/\b(pain|toast|bagel|croissant|pain grillé|pain grille)\b/,icon:"🍞"},{regex:/\b(chips|croustille|croustilles|nachos|popcorn|pop-corn)\b/,icon:"🥨"},{regex:/\b(chocolat|chocolat noir|chocolat blanc|chocolat au lait|choco)\b/,icon:"🍫"},{regex:/\b(légume|legume|carotte|brocoli|concombre|tomate|salade|poivron|céleri|celeri|haricot vert|courgette|champignon|avocat)\b/,icon:"🥗"}];return patterns.find(p=>p.regex.test(clean))?.icon||"🍎"}
