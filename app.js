(() => {
  ...
  function renderProfile() {
    ...
    if (Health && window.Capacitor?.getPlatform?.() === "ios") {
      ...
      waterSettingsSection?.insertAdjacentHTML("afterend", `<section class="card apple-health-profile-card"><h3>❤️ Apple Health</h3><p class="muted small">Importe le sommeil, les pas et les entraînements enregistrés dans Santé. Les champs restent accessibles dans Énergie et une correction manuelle n’est pas écrasée par la synchronisation suivante.</p><div class="apple-health-sync"><div class="apple-health-data"><span>😴 Sommeil</span><span>👟 Pas</span><span>🏃 Activités</span></div><small id="appleHealthStatus" class="apple-health-status">${esc(healthStatus)}</small><button class="secondary apple-health-sync-button" id="syncAppleHealth" type="button">Synchroniser</button></div><p class="muted tiny">Énergie lit seulement ces données et n’écrit rien dans Apple Health.</p></section>`);
