(() => {
  ...
  function buildMarie(store,offset,date,seed){
    ...
    if(missed)return;
    if(rand(seed+4)>.12) day.meals.push(meal("marie",date,late?"09:10":"06:35","Déjeuner",dairy?"Cappuccino et toast au beurre d’arachide":"Café noir, œufs et rôties",sleep>=7?4:2,sleep<6.5?["Fatigue"]:["Énergie stable"],sleep>=7?4:2));
    ...
    day.meals.push(meal("marie",date,"12:20","Dîner",lunch,water>=6?4:2,headache?["Maux de tête"]:(water<5?["Fatigue"]:["Satisfait"]),headache?2:4,headache?"Mal de tête apparu dans l’après-midi.":""));
    ...
    day.meals.push(meal("marie",date,"19:05","Souper",dinner,3,dairy&&rand(seed+6)<.2?["Ballonnements"]:["Satisfait"],dairy?3:4));
    if(rand(seed+8)<.32) day.meals.push(meal("marie",date,"15:40","Collation",dairy?"Latte et muffin":"Pomme et amandes",3,headache?["Maux de tête"]:["Énergie stable"],headache?2:4));
  }
  function buildAlex(store,offset,date,seed){
    ...
    if(missed)return;
    day.meals.push(meal("alex",date,"07:20","Déjeuner",seed%3===0?"Overnight oats, bleuets, chia et yogourt grec":"Œufs, pain complet, avocat et fruit",4,["Énergie stable"],4));
    day.meals.push(meal("alex",date,"12:10","Dîner",seed%2===0?"Poulet, quinoa, brocoli et poivrons":"Bol de tofu, riz brun et légumes",4,["Satisfait"],4));
    day.meals.push(meal("alex",date,"18:30","Souper",weekday===5?"Burger maison, pommes de terre et salade":"Saumon ou lentilles, légumes et riz",active?4:3,active?["Détendu"]:["Satisfait"],4));
    if(active||rand(seed+5)<.4)day.meals.push(meal("alex",date,"15:30","Collation","Banane, noix et fromage cottage",4,["Énergie stable"],4));
  }
  function buildSophie(store,offset,date,seed){
    ...
    day.meals.push(meal("sophie",date,"08:05","Déjeuner",highFiber?"Gruau, pomme, graines de chia et cannelle":"Rôties blanches et café",sleep>=7?3:2,sleep<6.5?["Fatigue"]:["Satisfait"],3));
    day.meals.push(meal("sophie",date,"12:35","Dîner",highFiber?"Salade de quinoa, pois chiches, concombre et feta":"Sandwich jambon-fromage et croustilles",3,[],3));
    const digestive=!highFiber&&rand(seed+7)<.42 || (highFiber&&phase<2&&rand(seed+7)<.22);
    day.meals.push(meal("sophie",date,"18:40","Souper",highFiber?(seed%2?"Chili aux haricots, riz brun et légumes":"Lentilles, légumes rôtis et quinoa"):(seed%2?"Pâtes sauce rosée":"Repas préparé et pain"),3,digestive?["Ballonnements","Mal de ventre"]:["Satisfait"],digestive?2:4,digestive?"Inconfort digestif en soirée.":""));
    if(rand(seed+5)<.35)day.meals.push(meal("sophie",date,"15:20","Collation",highFiber?"Pomme et amandes":"Biscuits et café",3,digestive?["Ballonnements"]:["Énergie stable"],digestive?2:4));
  }
